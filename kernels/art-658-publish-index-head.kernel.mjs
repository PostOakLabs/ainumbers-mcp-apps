...
// art-658-publish-index-head — pure structural gate over a §HEAD-1 head-commit publication event
// for an index/benchmark's per-stream publication history, with an independently-recomputed head_hash.
//
// From INDEX-LINEAGE-BUILD-SPEC.md's per-index-stream §HEAD-1 tip section. Publishes each
// rebalance cycle's evidence pack (art-646-compile-rebalance-evidence-pack) or correction event
// (art-647-record-index-correction) as one entry in a SPEC.md §HEAD-1 head-commit stream, so
// an index's published history becomes a verifiable, sequence-numbered, signer-continuous chain
// instead of a series of unlinked artifacts a reviewer must independently discover and order --
// mirroring the sibling NAV-lineage tokenized-fund stream section in NAV-LINEAGE-BUILD-SPEC.md
// and the model-risk validation stream (art-649-publish-model-risk-head), applied to an
// index/benchmark stream instead.
//
// BMR-scope amendment (INDEX-LINEAGE-BUILD-SPEC.md, 2026-08-20): Regulation (EU) 2025/914
// collapsed BMR scope to critical/significant (>=EUR50bn referenced), CTB/PAB, and certain
// commodity benchmarks -- the in-scope gate that decides WHICH administrator BMR duties bind is
// step zero for the REBAL/CORR rows this node cites, not for this node itself: this node is a
// pure structural head-commit gate that makes no scope determination of its own, and its copy
// carries the significant/critical + SEBI in-scope framing rather than a blanket BMR claim.
//
...
// independently re-derived by the citing node). This node does NOT itself execute an Ed25519
// verify: art-649's own build note records that the real zkVM guest has no WebCrypto at all, and
// even kernel-vm.mjs's host-bridged crypto.subtle simulation used for the VM<->worker parity gate
// was measured (while building art-649) to return a DIFFERENT verify() boolean than the same call
// running under Node -- i.e. an in-kernel "verified" claim would not be reproducible across the
// environments this repo requires byte-identical output from. What THIS node independently
// recomputes is head_hash -- pure SHA-256 digesting (via crypto.subtle.digest, confirmed
// byte-identical between worker and VM), never a caller-asserted value (SO #34): every function
// below (utf8ToBytes/cgCanon/jcsBytes/sha256Hex/securedHead/headHash) is copied VERBATIM from
// chaingraph/kernels/_head.mjs and _hash.mjs's cgCanon -- the SAME canonicalization, never a
// second one -- inlined rather than imported because every kernel in this tree runs unmodified
// inside kernel-vm.mjs's QuickJS sandbox for the mandatory VM<->worker parity gate, and that
// harness's ESM strip removes every import line except `{ executionHash } from './_hash.mjs'` (a
// hard, repo-wide invariant -- the art-476 FIX-2 "inline, don't narrow the import" lesson
// restated). If _head.mjs's headHash/cgCanon algorithm ever changes, this copy must move with it
// -- there is no automated linkage, same as _detmath's inline-never-import convention.
//
// A head-file tip proves the signer's claimed index-stream tip; it does NOT itself detect a
// signer publishing two conflicting index levels/weight sets for the same period (needs the
// ocg-head-tlog@1 witness backing, a later WU per the parent spec) and it is NOT itself a
// rebalance-cadence enforcement mechanism -- art-646/art-647 already record what changed and
// when; this stream only makes that history citable in sequence order.
...
const TOOL_ID = 'art-658-publish-index-head';
const TOOL_VERSION = '1.0.0';

export const meta = {
  tool_id: TOOL_ID, tool_version: TOOL_VERSION,
  mcp_name: 'publish_index_head',
...
const NOT_PROVEN = [
  ...
  { item: 'Equivocation detection', detail: 'A head-file tip proves the signer\'s claimed index-stream tip at this seq; it does not itself detect the same signer publishing a different, conflicting index level or weight set for the same (stream, seq) elsewhere. That needs the ocg-head-tlog@1 witness backing (a later WU), not this node.' },
  { item: 'Rebalance / correction-cycle enforcement', detail: 'This node publishes one head-commit event citing a rebalance-evidence-pack (art-646) or correction (art-647) artifact. It does not itself schedule, trigger, or verify that a rebalance or correction cycle was executed correctly -- those artifacts, and the administrator\'s own methodology document, are out of scope for this node.' },
  { item: 'Root-artifact authenticity', detail: 'root is a caller-supplied execution_hash citing the period\'s art-646 rebalance-evidence-pack or art-647 correction artifact, asserted and digested into this receipt. This node performs no lookup against a live artifact store and does not itself verify that the cited hash corresponds to a real, still-valid upstream artifact.' },
  { item: 'BMR / SEBI compliance', detail: 'This receipt evidences one head-commit publication event for an index-level stream. It has no bearing on whether the underlying rebalance or correction was correctly executed and does not itself satisfy EU Benchmark Regulation 2016/1011 (as amended by Regulation (EU) 2025/914, which narrowed BMR scope to critical/significant benchmarks) or SEBI\'s benchmark-administrator framework -- those are administrator-level oversight-function obligations this document evidences a piece of, never fulfills. This node makes no determination of whether the citing administrator is itself in BMR/SEBI scope.' },
];
 ...
 * }
 * stream is conventionally "index-head:<index_id>" (INDEX-LINEAGE-BUILD-SPEC.md's §HEAD-1 tip section); never
 * enforced beyond non-empty string here -- the convention lives in the caller and this node's
 * own copy/fixtures, not as a kernel-side format check.
...
export function compute(pp) {
  ...
  if (structural_error) {
    compliance_flags.push('IDXHEAD_STRUCTURAL_ERROR');
  } else {
    compliance_flags.push('IDXHEAD_STRUCTURE_VALID');
    compliance_flags.push(is_genesis ? 'IDXHEAD_GENESIS' : 'IDXHEAD_CHAINED');
    compliance_flags.push(signature_valid ? 'IDXHEAD_SIGNATURE_CLAIMED_VALID' : 'IDXHEAD_SIGNATURE_CLAIMED_INVALID');
    if (pp.head.rotates_to) compliance_flags.push('IDXHEAD_ROTATION_ANNOUNCED');
    if (chain_claim_present) compliance_flags.push(chain_valid ? 'IDXHEAD_CHAIN_CLAIMED_VALID' : 'IDXHEAD_CHAIN_CLAIMED_INVALID');
  }
  ...
  const output_payload = {
    ...
    chain_verified_by,
    chain_errors,
    // FLAG-MIRROR-DOCTRINE (AUTHORING-STANDARD.md's flag-mirror section): compliance_flags is a
    // sibling of output_payload, not a member of it, so a chain gate cannot route on
    // IDXHEAD_STRUCTURAL_ERROR directly. errors mirrors that same condition into
    // output_payload -- non-empty exactly when structural_error fired, empty otherwise.
    ...
    regulatory_framework: 'SPEC.md §HEAD-1 (this estate\'s own head-commit primitive, PROV-HEAD-1, DONE/merged) applied to an index/benchmark administrator\'s publication stream (INDEX-LINEAGE-BUILD-SPEC.md §5); not itself a determination of BMR/SEBI scope or an RDARR-style compliance requirement.',
  };
