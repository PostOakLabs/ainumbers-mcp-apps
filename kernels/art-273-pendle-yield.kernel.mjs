...
const TOOL_ID      = 'art-273-pendle-yield';
...
// Pendle Finance yield decomposition: PT implied fixed yield + YT leverage analytics.
// PT (Principal Token): buys at discount, redeems at face (1.0) at maturity --> fixed yield.
// YT (Yield Token): receives all underlying yield until maturity --> levered variable exposure.
// Invariant: PT_price + YT_price = 1.0 (face value of one underlying unit).
// Source: Pendle Finance documentation 2024; Pendle v2 AMM whitepaper.
// ZERO PII: market prices and yield parameters only. No wallet address or personal data.
...
export function compute(pp) {
  ...
  const pt_price         = Math.max(0.000001, Math.min(0.999999, safeNum(pp.pt_price, 0.94)));
  const yt_price         = Math.max(0.000001, safeNum(pp.yt_price, 1 - pt_price));
  const days_to_maturity = Math.max(1, safeNum(pp.days_to_maturity, 180));
  const underlying_apy   = Math.max(0, safeNum(pp.underlying_apy_pct, 12));
  const investment_usd   = Math.max(0, safeNum(pp.investment_usd, 10000));

  const compliance_flags = [];

  // PT + YT invariant check
  const pt_yt_sum         = round6(pt_price + yt_price);
  const invariant_holds   = Math.abs(pt_yt_sum - 1.0) < 0.005;
  if (!invariant_holds)   compliance_flags.push('PT_YT_INVARIANT_VIOLATED');

  const years_to_maturity = days_to_maturity / 365;

  // PT implied fixed yield: (1/pt_price)^(365/days) - 1, expressed as %.
  // Uses _detmath.pow for determinism across V8 and QuickJS.
  const pt_implied_fixed_yield_pct = round4(
    (pow(1 / pt_price, 1 / years_to_maturity) - 1) * 100
  );

  // YT leverage and effective APY at current underlying yield
  const yt_leverage              = round4(1 / yt_price);
  const yt_levered_apy_pct       = round4(underlying_apy * yt_leverage);
  // YT break-even: total yield accrued over period equals YT cost
  // YT earns underlying_apy * years on 1 unit of face value; cost = yt_price
  // break-even: underlying_apy_pct * years_to_maturity / 100 = yt_price
  // => break_even_apy = yt_price / years_to_maturity * 100
  const yt_break_even_apy_pct    = round4(yt_price / years_to_maturity * 100);
  const yt_profitable_at_current = underlying_apy >= yt_break_even_apy_pct;
  // YT margin of safety (how much underlying can drop before YT breaks even)
  const yt_margin_of_safety_pct  = round4(underlying_apy - yt_break_even_apy_pct);

  // PT analytics: return from buying PT vs holding cash
  // PT discount = 1 - pt_price (as % of face value)
  const pt_discount_pct          = round4((1 - pt_price) * 100);
  // Return to maturity: (1 - pt_price) / pt_price * 100
  const pt_return_to_maturity_pct = round4((1 / pt_price - 1) * 100);
  // PT effective APR (simple, annualized)
  const pt_simple_apr_pct        = round4(pt_return_to_maturity_pct / years_to_maturity);

  // Notional exposure with investment_usd
  const pt_units_bought  = investment_usd > 0 ? round4(investment_usd / pt_price) : 0;
  const yt_units_bought  = investment_usd > 0 ? round4(investment_usd / yt_price) : 0;
  const pt_profit_at_mat = round2(pt_units_bought - investment_usd); // redeems at face
  // YT expected total payout (at current underlying APY held constant)
  const yt_payout_if_stable = round2(yt_units_bought * underlying_apy / 100 * years_to_maturity);
  const yt_net_pnl_if_stable = round2(yt_payout_if_stable - investment_usd);

  if (pt_implied_fixed_yield_pct < 0)       compliance_flags.push('NEGATIVE_IMPLIED_YIELD');
  if (yt_leverage > 50)                     compliance_flags.push('EXTREME_YT_LEVERAGE');
  if (!yt_profitable_at_current)            compliance_flags.push('YT_BELOW_BREAK_EVEN');
  if (days_to_maturity < 7)                 compliance_flags.push('SHORT_MATURITY');

  const output_payload = {
    pt_price: round6(pt_price),
    yt_price: round6(yt_price),
    pt_yt_sum,
    invariant_holds,
    days_to_maturity: round2(days_to_maturity),
    years_to_maturity: round4(years_to_maturity),
    underlying_apy_pct: round4(underlying_apy),
    // PT analytics
    pt_discount_pct,
    ...
    // Notional sizing (at investment_usd)
    investment_usd: round2(investment_usd),
    pt_units_bought,
    pt_profit_at_maturity_usd: pt_profit_at_mat,
    yt_units_bought,
    yt_payout_if_underlying_stable_usd: yt_payout_if_stable,
    yt_net_pnl_if_underlying_stable_usd: yt_net_pnl_if_stable,
