...
// art-612 -- ERC-2612 Permit Digest & Binding Verifier: pure decision kernel.
// ETHMATH-WAVE-BUILD-SPEC.md section 2: recomputes the EIP-712 typed-data digest a wallet
// actually signs for ERC-2612's Permit struct (gasless-approval rail used by USDC/DAI-style
// tokens), recovers the ECDSA signer from a caller-supplied signature, and reports whether the
// recovered signer binds to the caller-claimed owner -- "different typehash, same digest/recover
// plumbing" as the sibling art-590 (x402-eip712-digest-recomputer) / art-591
// (x402-signer-recovery-verifier) pair (SPEC-X402-CRYPTO-CORE-1-2026-08-09.md sections 4-5).
//
// secp256k1 signature-recovery + keccak256 are vendored (@noble/curves + @noble/hashes, MIT,
// pinned v2.2.0, (c) Paul Miller paulmillr.com) -- WebCrypto has no native secp256k1, so there is
// no zero-dep shortcut. The vendored block below is copied byte-identical from
// chaingraph/kernels/art-591-x402-signer-recovery-verifier.kernel.mjs's own inlined copy of the
// same bundle (art-424/art-587/art-590/art-591 precedent, spec section 3: no second vendoring;
// ETHMATH-WAVE-BUILD-SPEC.md's own header repeats this -- reuse chaingraph/kernels/_noble-secp256k1.bundle.mjs
// via the already-inlined copy, never a second vendoring pass). Inlined rather than imported per
// RIDER-KERNEL #6 / the art-476 lesson: the chaingraph/vm QuickJS guest's ESM-strip only expects
// a kernel to import from ./_hash.mjs, and compute() must stay fully synchronous -- narrowing an
// import is not enough when compute() would otherwise await a crypto call, so the whole
// synchronous pure-JS/BigInt vendored block is inlined instead.
...
// ── art-612 ERC-2612 permit-digest + binding logic ─────────────────────────────────────────
// ⛔⛔ Never a facilitator/proxy/settlement relay (spec section 2). Zero network. Operates only
// on caller-supplied bytes: the four EIP-712 domain fields, the five ERC-2612 Permit struct
// fields (owner/spender/value/nonce/deadline), and a signature. This kernel recomputes a digest
// and recovers a signer -- it makes no claim about on-chain nonce freshness, current allowance
// state, or deadline-vs-now (that framing lives in output copy below, not a fetched check).
//
// All four EIP-712 domain fields (name, version, chainId, verifyingContract) are MANDATORY
// caller inputs -- never defaulted or guessed (a guessed verifyingContract defeats domain
// separation, same rule as art-590). Missing/malformed input -> verdict INDETERMINATE with
// reasons, never a thrown exception and never a NaN/non-finite value.
//
// The ERC-2612 `nonce` field is a uint256 counter (unlike EIP-3009's bytes32 nonce in the
// sibling art-590/591 pair) -- handled as a uint256 word here, not a bytes32.
//
// ⛔ "Binds" / "does not bind" language only -- never "valid", "approved", or "authorized". A
// digest that recomputes and a signature that recovers to the claimed owner means the signature
// was produced by that owner's key over these exact struct fields; it says nothing about whether
// the permit has already been consumed, whether the nonce is still current on chain, or whether
// `deadline` has passed relative to now.
//
// The Permit typehash below is keccak256 of a fixed, byte-for-byte public-spec string and is
// self-checked at module load against the well-known reference value used across
// OpenZeppelin's ERC20Permit.sol / EIP-2612's own reference implementation. A mismatch throws at
// load time rather than silently signing the wrong digest (same self-check pattern as art-590).

const EIP712DOMAIN_TYPE_STRING = 'EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)';
const PERMIT_TYPE_STRING = 'Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)';

const EIP712DOMAIN_TYPEHASH = keccak_256(utf8ToBytes(EIP712DOMAIN_TYPE_STRING));
const PERMIT_TYPEHASH = keccak_256(utf8ToBytes(PERMIT_TYPE_STRING));

const EIP712DOMAIN_TYPEHASH_EXPECT = '8b73c3c69bb8fe3d512ecc4cf759cc79239f7b179b0ffacaa9a75d522b39400f';
const PERMIT_TYPEHASH_EXPECT = '6e71edae12b1b97f4d1f60370fef10105fa2faae0126114a169c64845d6126c9';

(function _typehashSelfCheck() {
  const a = bytesToHex_(EIP712DOMAIN_TYPEHASH);
  if (a !== EIP712DOMAIN_TYPEHASH_EXPECT) {
    throw new Error('art-612 EIP712Domain typehash self-check FAILED: got ' + a + ' expected ' + EIP712DOMAIN_TYPEHASH_EXPECT);
  }
  const b = bytesToHex_(PERMIT_TYPEHASH);
  if (b !== PERMIT_TYPEHASH_EXPECT) {
    throw new Error('art-612 Permit typehash self-check FAILED: got ' + b + ' expected ' + PERMIT_TYPEHASH_EXPECT);
  }
})();
...
// separator computation is identical regardless of which typed struct is signed, so this checks
// the encoding path this kernel shares with art-590, not a Permit-specific value).
(function _domainSeparatorSelfCheck() {
  ...
  if (got !== expect) {
    throw new Error('art-612 EIP-712 domain-separator self-check FAILED (Ether Mail spec vector): got ' + got + ' expected ' + expect);
  }
})();
...
// 32-byte hex value (r, s) -- accepts 0x-prefixed or bare, exactly 64 hex chars.
...
// Accepts a decimal string, 0x-hex string, or safe-integer number; returns a non-negative
// integer or null. Never throws. (Used for v/yParity/chainId-as-recovery-input, distinct from
// the uint256-BigInt path above which is used for the struct fields themselves.)
function _toNonNegInt(v) {
  if (typeof v === 'number') return (Number.isFinite(v) && Number.isInteger(v) && v >= 0) ? v : null;
  if (typeof v === 'string') {
    const s = v.trim();
    if (s === '') return null;
    if (/^0x[0-9a-fA-F]+$/.test(s)) { const n = parseInt(s, 16); return Number.isSafeInteger(n) ? n : null; }
    if (/^[0-9]+$/.test(s)) { const n2 = parseInt(s, 10); return Number.isSafeInteger(n2) ? n2 : null; }
    return null;
  }
  return null;
}
...
// Parses an accepted signature notation (same as art-591 section 5.1): a single `signature` hex
// blob (64-byte r||s, or 65-byte r||s||v with the recovery marker embedded), OR split `r`/`s`
// fields with the recovery marker supplied separately via `yParity`/`v`.
function _parseSignatureFields(pp) {
  const reasons = [];
  let rHex = null, sHex = null, embeddedV = null;

  if (typeof pp.signature === 'string' && pp.signature.trim() !== '') {
    const sig = _stripHexPrefix(pp.signature.trim());
    if (!/^[0-9a-fA-F]+$/.test(sig)) {
      reasons.push('signature must be a hex string');
    } else if (sig.length === 128) {
      rHex = sig.slice(0, 64);
      sHex = sig.slice(64, 128);
  ...
  return { rHex, sHex, embeddedV, reasons };
}

// Normalizes v/yParity/EIP-155 v to the bundle's recovery-id convention (0/1). Same rule as
// art-591 section 5.2: a wrong conversion here silently produces a wrong recovered address, not
// a crash -- each accepted notation is handled explicitly rather than guessed.
function _normalizeRecoveryId(pp, embeddedV) {
  const yParity = _toNonNegInt(pp.yParity);
  if (yParity === 0 || yParity === 1) {
    return { recoveryId: yParity, source: 'yParity', reasons: [] };
  }

  const v = pp.v !== undefined ? _toNonNegInt(pp.v) : embeddedV;
  const source = pp.v !== undefined ? 'v' : (embeddedV !== null ? 'embedded_in_signature' : null);
  ...
  if (v === 0 || v === 1) {
    return { recoveryId: v, source: source + ':raw_bit', reasons: [] };
  }
  if (v === 27 || v === 28) {
    return { recoveryId: v - 27, source: source + ':v27_28', reasons: [] };
  }
  if (v >= 35) {
    const chainId = _toNonNegInt(pp.chainId);
    if (chainId === null) {
      return { recoveryId: null, source: null, reasons: ['v=' + v + ' looks like an EIP-155 value (chainId*2+35/36) but chainId was not supplied'] };
    }
    const recid = v - (chainId * 2 + 35);
    if (recid !== 0 && recid !== 1) {
      return { recoveryId: null, source: null, reasons: ['v=' + v + ' is not consistent with EIP-155 chainId=' + chainId + ' (expected chainId*2+35 or +36)'] };
    }
...
/**
 * compute(pp) -- pure verify_erc2612_permit_binding kernel.
 * pp: {
 *   name?, version?, chainId?, verifyingContract?,          -- EIP-712 domain, all mandatory
 *   owner?, spender?, value?, nonce?, deadline?,             -- ERC-2612 Permit struct, all mandatory
 *   signature?,                                              -- 0x + 128 or 130 hex chars (r||s or r||s||v)
 *   r?, s?,                                                  -- alternative to signature: split 32-byte hex fields
 *   v?, yParity?,                                             -- recovery marker: raw bit, 27/28, or EIP-155
 * }
 */
export function compute(pp) {
  ...
  const owner = _normalizeAddress(pp.owner);
  const spender = _normalizeAddress(pp.spender);
  const value = _toUint256BigInt(pp.value);
  const nonce = _toUint256BigInt(pp.nonce);
  const deadline = _toUint256BigInt(pp.deadline);
  if (!owner) reasons.push('owner is required and must be a 20-byte hex address');
  if (!spender) reasons.push('spender is required and must be a 20-byte hex address');
  if (value === null) reasons.push('value is required and must be a non-negative uint256');
  if (nonce === null) reasons.push('nonce is required and must be a non-negative uint256 (ERC-2612 nonce is a counter, not bytes32)');
  if (deadline === null) reasons.push('deadline is required and must be a non-negative uint256 (unix timestamp)');

  const sigFields = _parseSignatureFields(pp);
  reasons.push(...sigFields.reasons);

  let recovery = { recoveryId: null, source: null, reasons: [] };
  if (sigFields.rHex && sigFields.sHex) {
    recovery = _normalizeRecoveryId(pp, sigFields.embeddedV);
    reasons.push(...recovery.reasons);
  }
  ...
  if (reasons.length > 0) {
    return {
      output_payload: {
        ...
        permit_typehash: '0x' + bytesToHex_(PERMIT_TYPEHASH),
        scope_note: SCOPE_NOTE,
      },
      compliance_flags: ['ERC2612_PERMIT_INDETERMINATE', 'ERC2612_MALFORMED_INPUT'],
    };
  }
  ...
  const domainSeparatorHex = '0x' + bytesToHex_(domainSeparatorBytes);
  const structHashHex = '0x' + bytesToHex_(structHashBytes);
  const digestHex = '0x' + bytesToHex_(digestBytes);

  let recoveredSigner = null;
  let recoveryReasons = [];
  try {
    const rBytes = hexToBytes_(sigFields.rHex);
    const sBytes = hexToBytes_(sigFields.sHex);
    const sig = secp256k1.Signature.fromBytes(concatBytes_(rBytes, sBytes), 'compact').addRecoveryBit(recovery.recoveryId);
    const Q = sig.recoverPublicKey(digestBytes);
    const pubUncompressed = Q.toBytes(false); // 65 bytes: 0x04 prefix + 64-byte X||Y
    const addressBytes = keccak_256(pubUncompressed.subarray(1));
    recoveredSigner = '0x' + bytesToHex_(addressBytes.subarray(addressBytes.length - 20));
  } catch (e) {
    recoveryReasons.push('signature recovery failed: ' + (e && e.message ? e.message : String(e)));
  }
  ...
  if (!recoveredSigner) {
    return {
      output_payload: {
        ...
        domain: domain_echo,
        permit: permit_echo,
        domain_separator: domainSeparatorHex,
        struct_hash: structHashHex,
        digest: digestHex,
        recovered_signer: null,
        recovered_signer_matches_owner: null,
        recovery_id: recovery.recoveryId,
        recovery_id_source: recovery.source,
        domain_typehash: '0x' + bytesToHex_(EIP712DOMAIN_TYPEHASH),
        permit_typehash: '0x' + bytesToHex_(PERMIT_TYPEHASH),
        scope_note: SCOPE_NOTE,
      },
      compliance_flags: ['ERC2612_RECOVERY_FAILED'],
    };
  }

  const matches = recoveredSigner === owner;
  const verdict = matches ? 'BINDS' : 'DOES_NOT_BIND';
  const flags = matches
    ? ['ERC2612_PERMIT_BINDS']
    : ['ERC2612_PERMIT_DOES_NOT_BIND', 'ERC2612_RECOVERED_SIGNER_MISMATCH'];

  const output_payload = {
    verdict,
    reasons: [],
    diverging_field: matches ? null : 'owner',
    domain: domain_echo,
    permit: permit_echo,
    domain_separator: domainSeparatorHex,
    struct_hash: structHashHex,
    ...
    recovered_signer_matches_owner: matches,
    recovery_id: recovery.recoveryId,
    recovery_id_source: recovery.source,
    domain_typehash: '0x' + bytesToHex_(EIP712DOMAIN_TYPEHASH),
    permit_typehash: '0x' + bytesToHex_(PERMIT_TYPEHASH),
