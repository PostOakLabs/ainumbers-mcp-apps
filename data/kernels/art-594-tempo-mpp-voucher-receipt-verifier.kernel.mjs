...
// art-594-tempo-mpp-voucher-receipt-verifier: pure decision kernel.
// MPP-VERIFY-1 (widened to MPP-EVIDENCE-1 scope 2026-08-10). Full design rationale, sources,
// and red-line discussion are in the header comment further below (kept with the logic it
// documents rather than duplicated here).
//
// secp256k1 signature-recovery + keccak256 are vendored (@noble/curves + @noble/hashes, MIT,
// pinned v2.2.0, (c) Paul Miller paulmillr.com) -- WebCrypto has no native secp256k1, so there
// is no zero-dep shortcut. The vendored block below is copied byte-identical from
// chaingraph/kernels/_noble-secp256k1.bundle.mjs (same bundle already inlined by
// art-590/591/592/593/587 -- no second vendoring). Inlined rather than imported per
// RIDER-KERNEL #6 / the art-476 lesson: the chaingraph/vm QuickJS guest's ESM-strip only
// expects a kernel to import from ./_hash.mjs, and compute() must stay fully synchronous.
// DO NOT hand-edit the vendored block -- regenerate from chaingraph/kernels/_noble-secp256k1.bundle.mjs.
...
function utf8ToBytes(str) {
    ...
    // kernel's EIP712Domain typehash constant calls utf8ToBytes at module top level, so the
    // old body crashed at module load, before compute() ever ran). Pure-JS UTF-8 encoder,
...
// ── art-594 Tempo MPP voucher/receipt logic ─────────────────────────────────────────────────
// MPP-VERIFY-1 (widened to MPP-EVIDENCE-1 scope, robert adjudication 2026-08-10, Tim-approved).
// Sources, verified live 2026-08-10 (github.com/tempoxyz repos, MIT/Apache-2, design-read + cite
// safe per row licensing note):
//   - github.com/tempoxyz/mpp-specs specs/core/draft-httpauth-payment-00.md (WWW-Authenticate /
//     Payment / Payment-Receipt header shapes, HTTP 402 core scheme)
//   - github.com/tempoxyz/mpp-specs specs/methods/tempo/draft-tempo-session-00.md (EIP-712
//     domain, Voucher struct, server verification algorithm steps, receipt fields)
//   - github.com/tempoxyz/tempo tips/verify/src/TIP20ChannelReserve.sol +
//     interfaces/ITIP20ChannelReserve.sol (PRIMARY SOURCE: VOUCHER_TYPEHASH constant,
//     ChannelState struct field names or `settled`/`deposit`/`closeRequestedAt`, settle()'s
//     strict-increase revert AmountNotIncreasing)
// Voucher struct (confirmed against the primary Solidity source, not just mpp.dev's rendered
// docs): `Voucher(bytes32 channelId,uint96 cumulativeAmount)` for the v2 / "TIP20 Channel
// Reserve" domain (the shipped tips/verify reference implementation). mpp.dev's docs also
// describe a v1 "Tempo Stream Channel" domain with a uint128 amount word -- this kernel
// supports both via protocolVersion, but NEVER guesses which one: the domain name/version and
// protocolVersion are all caller-supplied, never defaulted (same discipline as
// art-590-x402-eip712-digest-recomputer's domain fields).
//
// ⛔⛔ RED LINES (unmoved, Tim 2026-08-10 widening ruling): this kernel never holds a private
// key, never signs anything, never escrows third-party funds, and never initiates a payment. It
// only verifies signatures and channel state the caller already supplies, and renders/validates
// the SHAPE of a 402 challenge and a Payment-Receipt object -- it never operates a live HTTP
// endpoint, never calls out, never resolves anything independently. "recipient" /
// "escrowContract" are caller-declared strings this node echoes, never addresses it controls or
// interacts with. A receiving address is not custody (browserchain-build-spec-v0.2.md section
// 21.5 precedent, "the relay holds only a receiving address, no hot keys").
//
// Three legs, each independently optional except the voucher leg's own internal completeness:
//   1. VOUCHER  -- cumulative EIP-712 voucher verification, ecrecover offline, no RPC. Mirrors
//      the mpp-specs session verification algorithm (recover signer -> enforce canonical low-s
//      -> match authorizedSigner/payer -> channel not closing -> monotonicity vs `settled` ->
//      idempotent-retry at equality -> minVoucherDelta -> deposit cap). The IDEMPOTENT_RETRY
//      verdict at cumulativeAmount === channelSettled *is* this tool's "Payment-Receipt
//      idempotency artifact": resubmitting the identical voucher always re-derives the identical
//      digest/verdict/receipt, never double-counts.
//   2. MEMO -- TIP-20 memo-validator (dormant candidate, folded in per this row). Structural-only
//      validation of the 32-byte memo reference: well-formedness, zero-memo convention, optional
//      equality check against a caller-declared expected reference, optional UTF-8 decodability
//      report. Never interprets on-chain effects (SPEC02:669-673 "cite not borrow" -- the memo
//      stays an opaque reference here exactly as the network treats it).
//   3. CHALLENGE + RECEIPT -- merchant-side: validates/renders a conformant WWW-Authenticate:
//      Payment challenge object (core spec fields) for a caller-declared price/recipient, then --
//      only if a VALID or IDEMPOTENT_RETRY voucher is also supplied and its domain/channel match
//      the challenge -- emits a Payment-Receipt-shaped object binding the voucher to an optional
//      caller-supplied subject_execution_hash (the computation being paid for). challenge.id is
//      ALWAYS caller-supplied (this node never mints an authoritative/HMAC-bound id, since doing
//      so would imply holding a server secret -- out of scope, never built).

function _stripHexPrefix(hex) {
  var s = String(hex == null ? '' : hex);
  return (s.slice(0, 2) === '0x' || s.slice(0, 2) === '0X') ? s.slice(2) : s;
}

function _normalizeAddress(v) {
  if (typeof v !== 'string') return null;
  var s = _stripHexPrefix(v.trim());
  if (!/^[0-9a-fA-F]{40}$/.test(s)) return null;
  return '0x' + s.toLowerCase();
}

function _normalize32ByteHex(v) {
  if (typeof v !== 'string') return null;
  var s = _stripHexPrefix(v.trim());
...
function _toNonNegInt(v) {
  if (typeof v === 'number') return (Number.isFinite(v) && Number.isInteger(v) && v >= 0) ? v : null;
  if (typeof v === 'string') {
    var s = v.trim();
    if (s === '') return null;
    if (/^0x[0-9a-fA-F]+$/.test(s)) { var n = parseInt(s, 16); return Number.isSafeInteger(n) ? n : null; }
    if (/^[0-9]+$/.test(s)) { var n2 = parseInt(s, 10); return Number.isSafeInteger(n2) ? n2 : null; }
    return null;
  }
  return null;
}

// Accepts a decimal string, 0x-hex string, or safe-integer number; returns a BigInt bounded to
// [0, 2^bits) or null. Never throws.
function _toBoundedBigInt(v, bits) {
  try {
    var bi;
    ...
    var ceiling = 1n << BigInt(bits);
    if (bi < 0n || bi >= ceiling) return null;
    return bi;
  } catch (e) { return null; }
}
...
function _addressWord(addrHex) {
  var bytes = hexToBytes_(_stripHexPrefix(addrHex));
  var out = new Uint8Array(32);
  out.set(bytes, 32 - bytes.length);
  return out;
}

function _bytes32Word(hex64) {
  return hexToBytes_(hex64);
}

// Parses an accepted signature notation (same shape art-591 established): a single `signature`
// hex blob (64-byte r||s, or 65-byte r||s||v with the recovery marker embedded), OR split
// r/s fields with the recovery marker supplied separately via yParity/v.
function _parseSignatureFields(pp) {
  var reasons = [];
  var rHex = null, sHex = null, embeddedV = null;
  if (typeof pp.signature === 'string' && pp.signature.trim() !== '') {
    var sig = _stripHexPrefix(pp.signature.trim());
    if (!/^[0-9a-fA-F]+$/.test(sig)) {
      reasons.push('signature must be a hex string');
    } else if (sig.length === 128) {
      rHex = sig.slice(0, 64); sHex = sig.slice(64, 128);
    } else if (sig.length === 130) {
      rHex = sig.slice(0, 64); sHex = sig.slice(64, 128);
      embeddedV = parseInt(sig.slice(128, 130), 16);
    } else {
      reasons.push('signature must be 64 bytes (r||s) or 65 bytes (r||s||v) hex, got ' + (sig.length / 2) + ' bytes');
    }
  } else {
    rHex = _normalize32ByteHex(pp.r);
    sHex = _normalize32ByteHex(pp.s);
    if (!rHex) reasons.push('r is required and must be a 32-byte hex value when signature is not supplied');
    if (!sHex) reasons.push('s is required and must be a 32-byte hex value when signature is not supplied');
  }
  return { rHex: rHex, sHex: sHex, embeddedV: embeddedV, reasons: reasons };
}

function _normalizeRecoveryId(pp, embeddedV) {
  var yParity = _toNonNegInt(pp.yParity);
  if (yParity === 0 || yParity === 1) return { recoveryId: yParity, source: 'yParity', reasons: [] };
  var v = (pp.v !== undefined && pp.v !== '') ? _toNonNegInt(pp.v) : embeddedV;
  var source = (pp.v !== undefined && pp.v !== '') ? 'v' : (embeddedV !== null ? 'embedded_in_signature' : null);
  if (v === null || v === undefined) {
    return { recoveryId: null, source: null, reasons: ['recovery id required: supply yParity, v, or a 65-byte signature with an embedded v byte'] };
  }
  if (v === 0 || v === 1) return { recoveryId: v, source: source + ':raw_bit', reasons: [] };
  if (v === 27 || v === 28) return { recoveryId: v - 27, source: source + ':v27_28', reasons: [] };
  if (v >= 35) {
    var chainId = _toNonNegInt(pp.domainChainId);
    if (chainId === null) return { recoveryId: null, source: null, reasons: ['v=' + v + ' looks like an EIP-155 value (chainId*2+35/36) but domainChainId was not supplied'] };
    var recid = v - (chainId * 2 + 35);
    if (recid !== 0 && recid !== 1) return { recoveryId: null, source: null, reasons: ['v=' + v + ' is not consistent with EIP-155 domainChainId=' + chainId + ' (expected chainId*2+35 or +36)'] };
    return { recoveryId: recid, source: source + ':eip155', reasons: [] };
  }
  return { recoveryId: null, source: null, reasons: ['v must be 0/1 (raw bit), 27/28 (legacy), or >=35 (EIP-155 chainId*2+35/36); got ' + v] };
}

var VOUCHER_TYPE_V1 = 'Voucher(bytes32 channelId,uint128 cumulativeAmount)';
var VOUCHER_TYPE_V2 = 'Voucher(bytes32 channelId,uint96 cumulativeAmount)';
var EIP712DOMAIN_TYPE_STRING = 'EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)';
var EIP712DOMAIN_TYPEHASH = keccak_256(utf8ToBytes(EIP712DOMAIN_TYPE_STRING));
var EIP712DOMAIN_TYPEHASH_EXPECT = '8b73c3c69bb8fe3d512ecc4cf759cc79239f7b179b0ffacaa9a75d522b39400f';
(function _domainTypehashSelfCheck() {
  var got = bytesToHex_(EIP712DOMAIN_TYPEHASH);
  if (got !== EIP712DOMAIN_TYPEHASH_EXPECT) {
    throw new Error('art-594 EIP712Domain typehash self-check FAILED: got ' + got + ' expected ' + EIP712DOMAIN_TYPEHASH_EXPECT + ' (same constant art-590 already verified -- shared across every EIP-712 kernel in this estate)');
  }
})();

var TEMPO_MPP_SCOPE_NOTE = 'Verifies a Tempo Machine Payments Protocol (MPP) session voucher offline (ecrecover, no RPC/database lookup), validates a TIP-20 32-byte memo structurally, and validates/renders a merchant-side HTTP 402 Payment challenge plus a Payment-Receipt-shaped binding to an optional subject execution_hash. Never holds a private key, never signs, never escrows third-party funds, never initiates a payment, never operates a live endpoint -- every field is caller-supplied and echoed, nothing is fetched or resolved independently. Sources: github.com/tempoxyz/mpp-specs (MIT/Apache-2) and github.com/tempoxyz/tempo tips/verify/src/TIP20ChannelReserve.sol (primary contract source for the Voucher typehash and ChannelState fields).';

// ── Leg 1: voucher verification ─────────────────────────────────────────────────────────────
function _verifyVoucher(v) {
  v = (v !== null && typeof v === 'object') ? v : {};
  var reasons = [];

  var hasAnyVoucherField = Object.keys(v).length > 0;
  if (!hasAnyVoucherField) {
    return { verdict: 'VOUCHER_SKIPPED', reasons: [], digest: null, recoveredSigner: null, expectedSigner: null,
      cumulativeAmount: null, channelSettled: null, channelDeposit: null, deltaAmount: null,
      protocolVersion: null, domainVerifyingContract: null, channelId: null, onChainSettleNote: null };
  }

  var protocolVersion = (v.protocolVersion === 'v1') ? 'v1' : (v.protocolVersion === 'v2' ? 'v2' : null);
  if (!protocolVersion) reasons.push('protocolVersion is required and must be "v1" (legacy "Tempo Stream Channel" domain, uint128 amount) or "v2" (the shipped "TIP20 Channel Reserve" reference contract, uint96 amount) -- never guessed');

  var name = _nonEmptyString(v.domainName);
  var version = _nonEmptyString(v.domainVersion);
  var chainId = _toBoundedBigInt(v.domainChainId, 256);
  var verifyingContract = _normalizeAddress(v.domainVerifyingContract);
  if (!name) reasons.push('domainName is required (EIP-712 domain field, never defaulted -- e.g. "TIP20 Channel Reserve" for v2)');
  if (!version) reasons.push('domainVersion is required (EIP-712 domain field, never defaulted)');
  if (chainId === null) reasons.push('domainChainId is required and must be a non-negative integer (EIP-712 domain field, never defaulted)');
  if (!verifyingContract) reasons.push('domainVerifyingContract is required and must be a 20-byte hex address (the deployed channel-reserve contract -- a guessed value defeats domain separation)');

  var channelId = _normalize32ByteHex(v.channelId);
  if (!channelId) reasons.push('channelId is required and must be a 32-byte hex value');

  var amountBits = protocolVersion === 'v1' ? 128 : 96;
  var cumulativeAmount = _toBoundedBigInt(v.cumulativeAmount, amountBits);
  if (cumulativeAmount === null) reasons.push('cumulativeAmount is required and must be a non-negative integer fitting uint' + amountBits + (protocolVersion ? '' : ' (width depends on protocolVersion)'));

  var channelSettled = _toBoundedBigInt(v.channelSettled, 96);
  if (channelSettled === null) reasons.push('channelSettled is required (the channel\'s on-file settled/highest-accepted cumulative amount, uint96 per ITIP20ChannelReserve.sol ChannelState -- supply "0" for a fresh channel)');
  var channelDeposit = _toBoundedBigInt(v.channelDeposit, 96);
  if (channelDeposit === null) reasons.push('channelDeposit is required (the channel\'s funded deposit cap, uint96)');
  var channelCloseRequestedAt = _toNonNegInt(v.channelCloseRequestedAt);
  if (channelCloseRequestedAt === null) reasons.push('channelCloseRequestedAt is required (uint32 timestamp; supply "0" for an open channel)');

  var authorizedSigner = v.channelAuthorizedSigner ? _normalizeAddress(v.channelAuthorizedSigner) : null;
  if (v.channelAuthorizedSigner && !authorizedSigner) reasons.push('channelAuthorizedSigner, if supplied, must be a 20-byte hex address');
  var payer = v.channelPayer ? _normalizeAddress(v.channelPayer) : null;
  if (v.channelPayer && !payer) reasons.push('channelPayer, if supplied, must be a 20-byte hex address');
  if (!authorizedSigner && !payer) reasons.push('either channelAuthorizedSigner or channelPayer is required, to know which address a voucher must be signed by');

  var minVoucherDelta = 0n;
  if (v.minVoucherDelta !== undefined && v.minVoucherDelta !== '' && v.minVoucherDelta !== null) {
    minVoucherDelta = _toBoundedBigInt(v.minVoucherDelta, 96);
    if (minVoucherDelta === null) reasons.push('minVoucherDelta, if supplied, must be a non-negative integer');
  }

  var sigFields = _parseSignatureFields(v);
  reasons = reasons.concat(sigFields.reasons);
  var recovery = { recoveryId: null, source: null, reasons: [] };
  if (sigFields.rHex && sigFields.sHex) {
    recovery = _normalizeRecoveryId({ v: v.v, yParity: v.yParity, domainChainId: v.domainChainId }, sigFields.embeddedV);
    reasons = reasons.concat(recovery.reasons);
  }

  if (reasons.length > 0) {
    return { verdict: 'VOUCHER_INDETERMINATE', reasons: reasons, digest: null, recoveredSigner: null,
      expectedSigner: null, cumulativeAmount: cumulativeAmount !== null ? cumulativeAmount.toString() : null,
      channelSettled: channelSettled !== null ? channelSettled.toString() : null,
      channelDeposit: channelDeposit !== null ? channelDeposit.toString() : null, deltaAmount: null,
      protocolVersion: protocolVersion, domainVerifyingContract: verifyingContract, channelId: channelId ? '0x' + channelId : null,
      onChainSettleNote: null };
  }

  var typeString = protocolVersion === 'v1' ? VOUCHER_TYPE_V1 : VOUCHER_TYPE_V2;
  var voucherTypehash = keccak_256(utf8ToBytes(typeString));

  var domainSeparatorBytes = keccak_256(concatBytes_(
  ...
  var structHashBytes = keccak_256(concatBytes_(
    voucherTypehash,
    _bytes32Word(channelId),
    _uint256Word(cumulativeAmount)
  ));
  var digestBytes = keccak_256(concatBytes_(Uint8Array.from([0x19, 0x01]), domainSeparatorBytes, structHashBytes));
  var digestHex = bytesToHex_(digestBytes);

  var rBytes = hexToBytes_(sigFields.rHex);
  var sBytes = hexToBytes_(sigFields.sHex);

  var recoveredSigner = null;
  var isHighS = false;
  var recoveryFailReasons = [];
  try {
    var sig = secp256k1.Signature.fromBytes(concatBytes_(rBytes, sBytes), 'compact');
    isHighS = sig.hasHighS();
    var sigRec = sig.addRecoveryBit(recovery.recoveryId);
    var Q = sigRec.recoverPublicKey(digestBytes);
    var pubUncompressed = Q.toBytes(false);
    var addressBytes = keccak_256(pubUncompressed.subarray(1));
    recoveredSigner = '0x' + bytesToHex_(addressBytes.subarray(addressBytes.length - 20));
  } catch (e) {
    recoveryFailReasons.push('signature recovery failed: ' + (e && e.message ? e.message : String(e)));
  }

  var expectedSigner = authorizedSigner || payer;

  if (!recoveredSigner) {
    return { verdict: 'VOUCHER_RECOVERY_FAILED', reasons: recoveryFailReasons, digest: '0x' + digestHex,
      recoveredSigner: null, expectedSigner: expectedSigner, cumulativeAmount: cumulativeAmount.toString(),
      channelSettled: channelSettled.toString(), channelDeposit: channelDeposit.toString(), deltaAmount: null,
      protocolVersion: protocolVersion, domainVerifyingContract: verifyingContract, channelId: '0x' + channelId,
      onChainSettleNote: null };
  }

  if (isHighS) {
    return { verdict: 'VOUCHER_NON_CANONICAL_SIGNATURE', reasons: ['signature s value is not canonical (high-S) -- rejected per the MPP session spec’s "enforce canonical (low-s) signatures" step'],
      digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
      cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
      channelDeposit: channelDeposit.toString(), deltaAmount: null, protocolVersion: protocolVersion,
      domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: null };
  }

  if (recoveredSigner !== expectedSigner) {
    return { verdict: 'VOUCHER_SIGNER_MISMATCH', reasons: ['recovered signer does not match channelAuthorizedSigner/channelPayer'],
      digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
      cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
      channelDeposit: channelDeposit.toString(), deltaAmount: null, protocolVersion: protocolVersion,
      domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: null };
  ...
  if (channelCloseRequestedAt !== 0) {
    return { verdict: 'VOUCHER_CHANNEL_CLOSING', reasons: ['channelCloseRequestedAt is nonzero -- the channel has an outstanding close request, per the session spec’s "channel.closeRequestedAt == 0" step'],
      digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
      cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
      channelDeposit: channelDeposit.toString(), deltaAmount: null, protocolVersion: protocolVersion,
      domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: null };
  ...
  var onChainSettleNote = 'Off-chain MPP session acceptance and on-chain TIP20ChannelReserve.settle() apply the SAME cumulativeAmount at two different layers with different strictness: settle() reverts AmountNotIncreasing when cumulativeAmount <= channel.settled (strict increase only), while the off-chain session server treats cumulativeAmount === settled as an idempotent retry (safe to accept, no double-charge). This node reports the off-chain verdict.';

  if (cumulativeAmount < channelSettled) {
    return { verdict: 'VOUCHER_STALE_AMOUNT', reasons: ['cumulativeAmount is below the channel’s on-file settled amount -- a stale or reordered voucher'],
      digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
      cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
      channelDeposit: channelDeposit.toString(), deltaAmount: '0', protocolVersion: protocolVersion,
      domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: onChainSettleNote };
  }

  if (cumulativeAmount === channelSettled) {
    return { verdict: 'VOUCHER_IDEMPOTENT_RETRY', reasons: [],
      digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
      cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
      channelDeposit: channelDeposit.toString(), deltaAmount: '0', protocolVersion: protocolVersion,
      domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: onChainSettleNote };
  ...
  var deltaAmount = cumulativeAmount - channelSettled;

  if (deltaAmount < minVoucherDelta) {
    return { verdict: 'VOUCHER_INSUFFICIENT_DELTA', reasons: ['delta since last settled amount is below minVoucherDelta'],
      digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
      cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
      channelDeposit: channelDeposit.toString(), deltaAmount: deltaAmount.toString(), protocolVersion: protocolVersion,
      domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: null };
  }

  if (cumulativeAmount > channelDeposit) {
    return { verdict: 'VOUCHER_EXCEEDS_DEPOSIT', reasons: ['cumulativeAmount exceeds channelDeposit -- the channel is not funded to cover this voucher'],
      digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
      cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
      channelDeposit: channelDeposit.toString(), deltaAmount: deltaAmount.toString(), protocolVersion: protocolVersion,
      domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: null };
  ...
  return { verdict: 'VOUCHER_VALID', reasons: [],
    digest: '0x' + digestHex, recoveredSigner: recoveredSigner, expectedSigner: expectedSigner,
    cumulativeAmount: cumulativeAmount.toString(), channelSettled: channelSettled.toString(),
    channelDeposit: channelDeposit.toString(), deltaAmount: deltaAmount.toString(), protocolVersion: protocolVersion,
    domainVerifyingContract: verifyingContract, channelId: '0x' + channelId, onChainSettleNote: null };
...
function _validateMemo(m) {
  ...
  if (m.value === undefined || m.value === null || m.value === '') {
    return { verdict: 'MEMO_SKIPPED', reasons: [], value: null, isZero: null, referenceMatch: null, decodesUtf8: null };
  }
  var hex = _normalize32ByteHex(m.value);
  if (!hex) {
    return { verdict: 'MEMO_MALFORMED', reasons: ['memo.value must be a 32-byte (bytes32) hex value per TIP-20’s fixed memo field'], value: String(m.value), isZero: null, referenceMatch: null, decodesUtf8: null };
  }
  var isZero = /^0+$/.test(hex);
  var reasons = [];
  var referenceMatch = null;
  if (m.expectedReferenceHash !== undefined && m.expectedReferenceHash !== null && m.expectedReferenceHash !== '') {
    var expHex = _normalize32ByteHex(m.expectedReferenceHash);
    if (!expHex) reasons.push('memo.expectedReferenceHash, if supplied, must be a 32-byte hex value');
    else referenceMatch = (expHex === hex);
  }
  var decodesUtf8 = null;
  if (m.expectedEncoding === 'utf8') {
    try {
      var bytes = hexToBytes_(hex);
  ...
  var verdict = 'MEMO_VALID';
  if (isZero) verdict = 'MEMO_EMPTY';
  else if (referenceMatch === false) verdict = 'MEMO_REFERENCE_MISMATCH';
  return { verdict: verdict, reasons: reasons, value: '0x' + hex, isZero: isZero, referenceMatch: referenceMatch, decodesUtf8: decodesUtf8 };
}

// ── Leg 3: merchant 402 challenge + Payment-Receipt emission ────────────────────────────────
function _buildChallenge(c) {
  c = (c !== null && typeof c === 'object') ? c : {};
  var hasAny = Object.keys(c).length > 0;
  if (!hasAny) return { verdict: 'CHALLENGE_SKIPPED', reasons: [], wwwAuthenticate: null, escrowContractNormalized: null, channelIdNormalized: null, amountNormalized: null };

  var reasons = [];
  var id = _nonEmptyString(c.id);
  if (!id) reasons.push('challenge.id is required and must be non-empty -- this node never mints an authoritative id (that would require holding a server-side HMAC secret, out of scope by the red lines), the caller’s own server issues and binds it per draft-httpauth-payment-00');
  var realm = _nonEmptyString(c.realm);
  if (!realm) reasons.push('challenge.realm is required');
  var method = c.method !== undefined && c.method !== '' ? c.method : 'tempo';
  if (method !== 'tempo') reasons.push('challenge.method must be "tempo" for this node (other MPP methods are out of scope)');
  var intent = c.intent;
  if (intent !== 'session' && intent !== 'charge') reasons.push('challenge.intent must be "session" or "charge"');
  var amount = (typeof c.amount === 'string' || typeof c.amount === 'number') && String(c.amount).trim() !== '' ? String(c.amount) : null;
  if (!amount) reasons.push('challenge.amount is required');
  var currency = _nonEmptyString(c.currency);
  if (!currency) reasons.push('challenge.currency is required');
  var recipient = _nonEmptyString(c.recipient);
  if (!recipient) reasons.push('challenge.recipient is required (a receiving address string -- never a private key; this node never holds or generates key material, and never becomes the merchant of record on the caller’s behalf)');
  var escrowContract = _normalizeAddress(c.escrowContract);
  if (!escrowContract) reasons.push('challenge.escrowContract is required and must be a 20-byte hex address (methodDetails.escrowContract per the Tempo session/charge method spec)');
  var channelId = c.channelId !== undefined && c.channelId !== '' ? _normalize32ByteHex(c.channelId) : null;
  if (c.channelId !== undefined && c.channelId !== '' && !channelId) reasons.push('challenge.channelId, if supplied, must be a 32-byte hex value');

  if (reasons.length > 0) {
    return { verdict: 'CHALLENGE_INDETERMINATE', reasons: reasons, wwwAuthenticate: null, escrowContractNormalized: escrowContract, channelIdNormalized: channelId ? '0x' + channelId : null, amountNormalized: amount };
  }
  ...
  var requestObject = {
    ...
    methodDetails: {
      ...
      channelId: channelId ? '0x' + channelId : null,
      minVoucherDelta: (c.minVoucherDelta !== undefined && c.minVoucherDelta !== '') ? String(c.minVoucherDelta) : null,
      chainId: (c.chainId !== undefined && c.chainId !== '') ? String(c.chainId) : null
    }
  };
  var wwwAuthenticate = { id: id, realm: realm, method: method, intent: intent, request: requestObject, expires: _nonEmptyString(c.expires) };

  return { verdict: 'CHALLENGE_WELLFORMED', reasons: [], wwwAuthenticate: wwwAuthenticate, escrowContractNormalized: escrowContract, channelIdNormalized: channelId ? '0x' + channelId : null, amountNormalized: amount };
}

function _buildReceipt(voucherResult, challengeResult, subjectExecutionHash, now) {
  if (challengeResult.verdict === 'CHALLENGE_SKIPPED') {
    return { verdict: 'RECEIPT_NOT_REQUESTED', receipt: null };
  }
  if (challengeResult.verdict !== 'CHALLENGE_WELLFORMED') {
    return { verdict: 'RECEIPT_WITHHELD_CHALLENGE_INDETERMINATE', receipt: null };
  }
  if (voucherResult.verdict !== 'VOUCHER_VALID' && voucherResult.verdict !== 'VOUCHER_IDEMPOTENT_RETRY') {
    return { verdict: 'RECEIPT_WITHHELD_VOUCHER_INVALID', receipt: null };
  }
  if (voucherResult.domainVerifyingContract !== challengeResult.escrowContractNormalized) {
    return { verdict: 'RECEIPT_WITHHELD_CONTRACT_MISMATCH', receipt: null };
  }
  if (challengeResult.channelIdNormalized && challengeResult.channelIdNormalized !== voucherResult.channelId) {
    return { verdict: 'RECEIPT_WITHHELD_CHANNEL_MISMATCH', receipt: null };
  }
  var delta = voucherResult.deltaAmount !== null ? BigInt(voucherResult.deltaAmount) : 0n;
  var amountRequired = (function () {
    try { return BigInt(challengeResult.amountNormalized); } catch (e) { return null; }
  })();
  if (voucherResult.verdict === 'VOUCHER_VALID' && amountRequired !== null && delta < amountRequired) {
    return { verdict: 'RECEIPT_WITHHELD_INSUFFICIENT_AMOUNT', receipt: null };
  }

  var receipt = {
    status: 'success',
    method: 'tempo',
    intent: challengeResult.wwwAuthenticate.intent,
    timestamp: now || null,
    reference: voucherResult.channelId,
    challengeId: challengeResult.wwwAuthenticate.id,
    channelId: voucherResult.channelId,
    acceptedCumulative: voucherResult.cumulativeAmount,
    spent: voucherResult.deltaAmount,
    voucherDigest: voucherResult.digest,
    subjectExecutionHash: subjectExecutionHash || null
  };
  return { verdict: 'RECEIPT_EMITTED', receipt: receipt };
}

function compute(pp) {
  pp = (pp !== null && typeof pp === 'object') ? pp : {};

  var voucherResult = _verifyVoucher({
    protocolVersion: pp.protocolVersion, domainName: pp.domainName, domainVersion: pp.domainVersion,
    domainChainId: pp.domainChainId, domainVerifyingContract: pp.domainVerifyingContract,
    channelId: pp.channelId, cumulativeAmount: pp.cumulativeAmount,
    signature: pp.signature, r: pp.r, s: pp.s, v: pp.v, yParity: pp.yParity,
    channelAuthorizedSigner: pp.channelAuthorizedSigner, channelPayer: pp.channelPayer,
    channelSettled: pp.channelSettled, channelDeposit: pp.channelDeposit,
    channelCloseRequestedAt: pp.channelCloseRequestedAt, minVoucherDelta: pp.minVoucherDelta
  });

  var memoResult = _validateMemo({
    value: pp.memoValue, expectedReferenceHash: pp.memoExpectedReferenceHash, expectedEncoding: pp.memoExpectedEncoding
  });

  var challengeResult = _buildChallenge({
    id: pp.challengeId, realm: pp.challengeRealm, method: pp.challengeMethod, intent: pp.challengeIntent,
    amount: pp.challengeAmount, currency: pp.challengeCurrency, recipient: pp.challengeRecipient,
    escrowContract: pp.challengeEscrowContract, channelId: pp.challengeChannelId,
    minVoucherDelta: pp.challengeMinVoucherDelta, chainId: pp.challengeChainId, expires: pp.challengeExpires
  });

  var receiptResult = _buildReceipt(voucherResult, challengeResult, pp.subjectExecutionHash, pp.now);

  var reasons = [].concat(voucherResult.reasons || [], memoResult.reasons || [], challengeResult.reasons || []);

  var flags = [];
  flags.push('TEMPO_MPP_VOUCHER_' + voucherResult.verdict.replace(/^VOUCHER_/, ''));
  if (memoResult.verdict !== 'MEMO_SKIPPED') flags.push('TIP20_' + memoResult.verdict);
  if (challengeResult.verdict !== 'CHALLENGE_SKIPPED') flags.push('TEMPO_MPP_' + challengeResult.verdict);
  if (receiptResult.verdict === 'RECEIPT_EMITTED') flags.push('TEMPO_MPP_RECEIPT_EMITTED');

  var output_payload = {
    voucher: {
      verdict: voucherResult.verdict, reasons: voucherResult.reasons, digest: voucherResult.digest,
      recoveredSigner: voucherResult.recoveredSigner, expectedSigner: voucherResult.expectedSigner,
      cumulativeAmount: voucherResult.cumulativeAmount, channelSettled: voucherResult.channelSettled,
      channelDeposit: voucherResult.channelDeposit, deltaAmount: voucherResult.deltaAmount,
      protocolVersion: voucherResult.protocolVersion, channelId: voucherResult.channelId,
      onChainSettleNote: voucherResult.onChainSettleNote
    },
    memo: {
      verdict: memoResult.verdict, reasons: memoResult.reasons, value: memoResult.value,
      isZero: memoResult.isZero, referenceMatch: memoResult.referenceMatch, decodesUtf8: memoResult.decodesUtf8
    },
    challenge: {
      verdict: challengeResult.verdict, reasons: challengeResult.reasons, wwwAuthenticate: challengeResult.wwwAuthenticate
    },
    receipt: { verdict: receiptResult.verdict, paymentReceipt: receiptResult.receipt },
    reasons: reasons,
    scope_note: TEMPO_MPP_SCOPE_NOTE
  };

  return { output_payload: output_payload, compliance_flags: flags };
}

const TOOL_ID = 'art-594-tempo-mpp-voucher-receipt-verifier';
...
/**
 * compute(pp) -- pure verify_tempo_mpp_voucher kernel.
 * pp: {
 *   // voucher leg (optional as a whole; if any field is present, all required fields must be)
 *   protocolVersion,                 -- 'v1' | 'v2', required if verifying a voucher
 *   domainName, domainVersion, domainChainId, domainVerifyingContract,  -- EIP-712 domain
 *   channelId, cumulativeAmount,
 *   signature?, r?, s?, v?, yParity?,
 *   channelAuthorizedSigner?, channelPayer?,   -- at least one required
 *   channelSettled, channelDeposit, channelCloseRequestedAt,
 *   minVoucherDelta?,
 *   // memo leg (optional)
 *   memoValue?, memoExpectedReferenceHash?, memoExpectedEncoding?,
 *   // challenge + receipt leg (optional)
 *   challengeId?, challengeRealm?, challengeMethod?, challengeIntent?, challengeAmount?,
 *   challengeCurrency?, challengeRecipient?, challengeEscrowContract?, challengeChannelId?,
 *   challengeMinVoucherDelta?, challengeChainId?, challengeExpires?,
 *   subjectExecutionHash?, now?
 * }
 */
