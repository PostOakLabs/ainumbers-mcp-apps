...
function randn(rng) {
  const u1 = rng(), u2 = rng();
  return Math.sqrt(-2 * det.log(u1 + 1e-15)) * det.cos(2 * Math.PI * u2);
}
...
export function compute(pp) {
  ...
  const discountFactor = t => det.exp(-rfr * t);
  const survCpy = t => det.exp(-cpyPD * t);

  let cva = 0, dva = 0, fva = 0;
  for (let t = 1; t <= nSteps; t++) {
    const t0 = times[t - 1], t1 = times[t];
    const dPD_cpy = det.exp(-cpyPD * t0) - det.exp(-cpyPD * t1);
    const dPD_own = det.exp(-ownPD  * t0) - det.exp(-ownPD  * t1);
