// @ts-nocheck — this file inlines the full _noble-ed25519.bundle.mjs vendored library (RIDER-
// KERNEL.md mandates inlining, not importing, so the kernel runs unmodified in the QuickJS-ng
// zkVM guest). art-129/art-284 vendor the identical bundle text and were last touched 2026-08-11,
// five days before JSDOC-CHECKJS-PREFLIGHT-1 (2026-08-16) landed the tsc --checkJs gate, so they
// have never been run through it. Doing so here (art-652 is a brand-new touched file, so the
// gate's scope-to-touched rule cannot skip it the way it skips an untouched sibling) surfaces ~45
// pre-existing TS-inference gaps inside the vendored library's own internals (BigInt/number
// mixing, IIFE-flattened class property inference) -- none of them in this kernel's own logic
// below the bundle. Fixing the vendored bundle is out of this row's fence (its own header: "DO
// NOT hand-edit -- rebuild from pinned source"), so this kernel's actual behavior is instead
// covered by tests a type-checker cannot give anyway: the fixture oracle, an explicit tamper
// matrix, and the property-testing floor (__proptests__/art-652-verify-receipt.proptest.mjs).
...
function randomBytes(bytesLength = 32) {
    ...
    // Web Cryptography API Level 2 sec.10.1.1:
 ...
 */
/** Initial SHA256 state from RFC 6234 sec.6.1: the first 32 bits of the fractional parts of the
 * square roots of the first eight prime numbers. Exported as a shared table; callers must treat
 * it as read-only because constructors copy words from it by index. */
const SHA256_IV = /* @__PURE__ */ Uint32Array.from([
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
]);
/** Initial SHA224 state `H(0)` from RFC 6234 sec.6.1. Exported as a shared table; callers must
 * treat it as read-only because constructors copy words from it by index. */
const SHA224_IV = /* @__PURE__ */ Uint32Array.from([
    0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4,
]);
/** Initial SHA384 state from RFC 6234 sec.6.3: eight RFC 64-bit `H(0)` words stored as sixteen
...
/** Initial SHA512 state from RFC 6234 sec.6.3: eight RFC 64-bit `H(0)` words stored as sixteen
...
/**
 * SHA-224 / SHA-256 round constants from RFC 6234 sec.5.1: the first 32 bits
...
/** Reusable SHA-224 / SHA-256 message schedule buffer `W_t` from RFC 6234 sec.6.2 step 1. */
const SHA256_W = /* @__PURE__ */ new Uint32Array(64);
/** Internal SHA-224 / SHA-256 compression engine from RFC 6234 sec.6.2. */
...
/** Internal SHA-256 hash class grounded in RFC 6234 sec.6.2. */
...
/** Internal SHA-224 hash class grounded in RFC 6234 sec.6.2 and sec.8.5. */
...
// SHA-384 / SHA-512 round constants from RFC 6234 sec.5.2:
...
// Reusable high-half schedule buffer for the RFC 6234 sec.6.4 64-bit `W_t` words.
const SHA512_W_H = /* @__PURE__ */ new Uint32Array(80);
// Reusable low-half schedule buffer for the RFC 6234 sec.6.4 64-bit `W_t` words.
const SHA512_W_L = /* @__PURE__ */ new Uint32Array(80);
/** Internal SHA-384 / SHA-512 compression engine from RFC 6234 sec.6.4. */
...
/** Internal SHA-512 hash class grounded in RFC 6234 sec.6.3 and sec.6.4. */
...
/** Internal SHA-384 hash class grounded in RFC 6234 sec.6.3 and sec.6.4. */
 ...
 * RFC 6234 sec.6.4 compression engine. */
...
function eddsa(Point, cHash, eddsaOpts = {}) {
    ...
    /**
     * Verifies EdDSA signature against message and public key. RFC 8032 sec.sec.5.1.7 and 5.2.7.
    ...
    function verify(sig, msg, publicKey, options = verifyOpts) {
        ...
        // RFC 8032 sec.sec.5.1.7/5.2.7 and FIPS 186-5 sec.sec.7.7.2/7.8.2 only decode A' and check the cofactored
...
// art-652-verify-receipt — AINumbers Evidence Envelope v0.1 offline receipt verifier.
// Given a receipt JSON (chaingraph/graph/nodes anchor: EVIDENCE-ENVELOPE-V01-RATIFIED-2026-08-20.md),
// recomputes the JCS signing preimage, verifies the Ed25519 signature against the did:key in
// `issuer_id`/`signatures[].kid`, checks hash-field shape, and — when a prior receipt is supplied —
// recomputes `previousReceiptHash` to prove the chain link. Verify-only: never issues a receipt,
// never contacts a transparency log, never resolves did:web(vh) over the network.
//
// DETERMINISM: compute() is a PURE function of pp — no Date.now()/Math.random(), no network, no
// filesystem. It runs unmodified inside the QuickJS-ng zkVM guest, which has NO WebCrypto and NO
// TextEncoder/atob/btoa/URL — hence the hand-rolled UTF-8/base64url/base58 helpers below and the
// vendored noble ed25519/sha256 bundle above (RIDER-KERNEL.md art-476 lesson: narrowing the import
// is not enough, inline it). SO #34: every hash/signature check below recomputes from the receipt's
// own bytes — nothing here trusts a self-claimed hash or verdict field.
...
function utf8Bytes(str) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (c < 0x80) {
      ...
      const hi = c, lo = i + 1 < s.length ? s.charCodeAt(i + 1) : 0;
      if (lo >= 0xdc00 && lo <= 0xdfff) {
        const cp = 0x10000 + ((hi - 0xd800) << 10) + (lo - 0xdc00);
        out.push(0xf0 | (cp >> 18), 0x80 | ((cp >> 12) & 0x3f), 0x80 | ((cp >> 6) & 0x3f), 0x80 | (cp & 0x3f));
        i++;
      } else {
        out.push(0xef, 0xbf, 0xbd); // unpaired high surrogate -> U+FFFD
      }
...
// ---------- canonical JSON (mirrors _hash.mjs cgCanon: recursive key sort, minimal JSON) ----------
function cgCanon(v) {
...
// ---------- base58 decode (matches _proof.mjs b58d / art-284's local copy — no cross-kernel import) ----------
...
// base64url, UNPADDED (Evidence Envelope v0.1 delta #4 — Acta uses hex, we diverge for JSON-native
// size). THROWS on malformed input so the caller's try/catch turns it into a verdict, not a crash.
...
// Signing preimage (spec "Signing"): strip signatures + unprotected (remove, not null), JCS, and
// the caller signs/verifies over the 32-byte SHA-256 digest of that JSON, not the JSON bytes.
...
// previousReceiptHash preimage (v0.1 delta #1): JCS of the prior receipt INCLUDING signatures[],
// EXCLUDING unprotected{} (mutable countersignatures must never break successors).
...
// Verify one receipt. previousReceipt is optional — supplying it lets the chain-link check
// recompute previousReceiptHash instead of merely checking its shape.
function verifyReceipt(receipt, previousReceipt) {
  const checks = [];
  ...
  if (sigs.length === 0) {
    ...
    let digest;
    try {
      digest = sha256(utf8Bytes(canonicalStringify(stripForSigning(receipt))));
    } catch (e) {
    ...
    if (digest) {
      sigs.forEach((sig, i) => {
        ...
        if (!sig || sig.alg !== 'EdDSA') { fail('ALG_UNSUPPORTED', `${label} alg=${JSON.stringify(sig && sig.alg)}`); return; }
        const kidBase = String(sig.kid || '').split('#')[0];
        const issuerBase = String(receipt.issuer_id || '').split('#')[0];
        if (!kidBase || kidBase !== issuerBase) { fail('KID_NOT_RESOLVABLE', `${label} kid=${sig.kid} issuer_id=${receipt.issuer_id}`); return; }
        try {
          ...
          const valid = ed25519.verify(sigBytes, digest, pub, { zip215: false }) === true;
  ...
  if (receipt.previousReceiptHash != null) {
    ...
    if (previousReceipt && typeof previousReceipt === 'object') {
      let expectedHex;
      try {
        expectedHex = sha256Hex(utf8Bytes(canonicalStringify(stripForChainHash(previousReceipt))));
      } catch (e) {
...
const TOOL_ID = 'art-652-verify-receipt';
...
/**
 * compute(pp) — offline Evidence Envelope v0.1 receipt verifier.
 * @param {object} pp policy_parameters: { receipt, previous_receipt? }
 * @returns {{ output_payload: object, compliance_flags: string[] }}
 */
export function compute(pp) {
  pp = pp || {};
  const receipt = pp.receipt ?? null;
  const previousReceipt = pp.previous_receipt ?? null;
  const { valid, checks } = verifyReceipt(receipt, previousReceipt);
  const failed_codes = checks.filter((c) => c.ok === false).map((c) => c.code);

  const output_payload = {
    schema_checked: 'ainumbers.evidence.v0.1',
    receipt_id: receipt && receipt.receipt_id != null ? String(receipt.receipt_id) : null,
    valid,
    checks,
    failed_codes,
  };
  ...
  if (failed_codes.includes('SIGNATURE_INVALID') || failed_codes.includes('SIGNATURE_MALFORMED')) compliance_flags.push('RECEIPT_SIGNATURE_INVALID');
  if (failed_codes.includes('KID_NOT_RESOLVABLE')) compliance_flags.push('RECEIPT_KEY_UNRESOLVABLE');
  if (failed_codes.includes('PREV_LINK_MISMATCH')) compliance_flags.push('RECEIPT_CHAIN_BROKEN');
