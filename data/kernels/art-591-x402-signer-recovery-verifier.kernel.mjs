...
// art-591 -- x402 Signer Recovery Verifier: pure decision kernel.
// SPEC-X402-CRYPTO-CORE-1-2026-08-09.md section 5: recovers the ECDSA signer address from an
// EIP-712 digest (the sibling art-590-x402-eip712-digest-recomputer's output) plus a signature
// in (r,s,v) / (r,s,yParity) form, and reports whether it matches a caller-claimed `from`
// address. Never a facilitator/proxy/settlement relay (spec section 2). Zero network -- every
// input is caller-supplied, nothing is fetched or resolved independently.
//
...
// chaingraph/kernels/art-587-finp2p-ledger-proof-verifier.kernel.mjs's own inlined copy of the
// same bundle (art-424/art-587 precedent, spec section 3: no second vendoring). Inlined rather
// than imported per RIDER-KERNEL #6 / the art-476 lesson: the chaingraph/vm QuickJS guest's
// ESM-strip only expects a kernel to import from ./_hash.mjs, and compute() must stay fully
// synchronous -- narrowing an import is not enough when compute() would otherwise await a
...
// ── art-591 signer-recovery logic ───────────────────────────────────────────────────────────
// SPEC-X402-CRYPTO-CORE-1-2026-08-09.md section 5: recovery-id normalization covers the
// raw bit (0/1), legacy Ethereum v (27/28), and EIP-155 v (chainId*2+35/36) -- each a
// well-known transcription trap the spec calls out by name. Malformed/unparseable signature
// bytes, an r/s outside curve order, or a recovery id that yields "point at infinity" are
// caught here and converted into a clean RECOVERY_FAILED finding, never an unhandled throw
// reaching the tool boundary (same rule X402LINT-FIX-1 already established for this estate).
...
// 32-byte hex value (digest, r, s) -- accepts 0x-prefixed or bare, exactly 64 hex chars.
...
// Parses an accepted signature notation (spec section 5.1): a single `signature` hex blob
// (64-byte r||s, or 65-byte r||s||v with the recovery marker embedded), OR split `r`/`s`
...
// Normalizes v/yParity/EIP-155 v to the bundle's recovery-id convention (0/1). Spec section
// 5.2: a wrong conversion here silently produces a wrong recovered address, not a crash --
// each accepted notation is handled explicitly rather than guessed.
function _normalizeRecoveryId(pp, embeddedV) {
  ...
  if (v >= 35) {
    var chainId = _toNonNegInt(pp.chainId);
    if (chainId === null) {
...
export function compute(pp) {
  ...
  var digestHex = _normalize32ByteHex(pp.digest);
  if (!digestHex) reasons.push('digest is required and must be a 32-byte hex value (the art-590 recomputed EIP-712 digest)');

  var claimedFrom = pp.claimedFrom !== undefined ? _normalizeAddress(pp.claimedFrom) : null;
  if (pp.claimedFrom !== undefined && pp.claimedFrom !== null && pp.claimedFrom !== '' && !claimedFrom) {
    reasons.push('claimedFrom, if supplied, must be a 20-byte hex address');
  }
  ...
  if (reasons.length > 0) {
    return {
      output_payload: {
        ...
        reasons: reasons,
        digest: digestHex ? '0x' + digestHex : null,
      ...
      compliance_flags: ['X402_RECOVER_INDETERMINATE', 'X402_MALFORMED_INPUT'],
    };
  }

  var digestBytes = hexToBytes_(digestHex);
  ...
  try {
    ...
    var Q = sig.recoverPublicKey(digestBytes);
  ...
  if (!recoveredSigner) {
    return {
      ...
      compliance_flags: ['X402_RECOVERY_FAILED'],
    };
  }

  var matches = claimedFrom !== null ? (claimedFrom === recoveredSigner) : null;
  var flags = ['X402_SIGNER_RECOVERED'];
  if (matches === true) flags.push('X402_RECOVERED_SIGNER_MATCHES_CLAIMED_FROM');
  if (matches === false) flags.push('X402_RECOVERED_SIGNER_MISMATCH');
  ...
  var output_payload = {
    ...
    claimed_from: claimedFrom,
    recovered_signer_matches_claimed_from: matches,
    recovery_id: recovery.recoveryId,
    recovery_id_source: recovery.source,
    scope_note: SCOPE_NOTE,
  };
...
const TOOL_ID = 'art-591-x402-signer-recovery-verifier';
...
/**
 * compute(pp) -- pure verify_x402_signer_recovery kernel.
 * pp: {
 *   digest,                                   -- required, 0x + 64 hex chars (art-590 output)
 ...
 *   chainId?,                                   -- required only to decode an EIP-155 v
