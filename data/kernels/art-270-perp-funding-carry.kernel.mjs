...
const log = _detmath.log;
/* ===== END inlined _detmath ===== */

const { pow } = _detmath;

const TOOL_ID      = 'art-270-perp-funding-carry';
...
// Venue-specific taker fee defaults (% of notional, e.g. 0.035 = 3.5 bps)
// Source: Hyperliquid docs 2024; Binance/Bybit/OKX fee schedules 2024.
const VENUE_FEES = {
  hyperliquid: { taker_pct: 0.035, cadence_hours: 1 },
  binance:     { taker_pct: 0.05,  cadence_hours: 8 },
  bybit:       { taker_pct: 0.055, cadence_hours: 8 },
  dydx_v4:     { taker_pct: 0.05,  cadence_hours: 8 },
  okx:         { taker_pct: 0.05,  cadence_hours: 8 },
};
const HOURS_PER_YEAR = 8760;
const HL_FUNDING_CAP_PCT_PER_HOUR = 4;
...
// Compound annual rate from per-hour rate: (1 + r/100)^8760 - 1 expressed as %.
// Uses _detmath.pow to guarantee bit-identical results across V8 and QuickJS.
function compoundAnnualPct(hourly_rate_pct) {
  const r = hourly_rate_pct / 100;
  if (r <= -1) return -100;
  const result = (pow(1 + r, HOURS_PER_YEAR) - 1) * 100;
...
export function compute(pp) {
  ...
  const venue     = (pp.venue || 'hyperliquid').toLowerCase();
  const venue_def = VENUE_FEES[venue] || VENUE_FEES.hyperliquid;
  const cadence_h = Math.max(0.0001, safeNum(pp.cadence_hours, venue_def.cadence_hours));
  const rate_pct  = safeNum(pp.funding_rate_pct_per_period, 0.01);
  const taker_pct = Math.max(0, safeNum(pp.taker_fee_pct, venue_def.taker_pct));
  const basis_spot = Math.max(0.000001, safeNum(pp.basis_spot_price, 60000));
  const basis_perp = Math.max(0.000001, safeNum(pp.basis_perp_price, basis_spot));

  const compliance_flags = [];

  // Normalise to hourly rate
  const hourly_rate_pct     = round6(rate_pct / cadence_h);
  const simple_annual_pct   = round4(hourly_rate_pct * HOURS_PER_YEAR);
  const compound_annual_pct = round4(compoundAnnualPct(hourly_rate_pct));

  // Basis analytics (contango/backwardation)
  const basis_usd            = round6(basis_perp - basis_spot);
  const basis_pct            = round6(basis_usd / basis_spot * 100);
  const periods_per_year     = HOURS_PER_YEAR / cadence_h;
  const annualized_basis_pct = round4(basis_pct * periods_per_year);

  if (Math.abs(rate_pct) > HL_FUNDING_CAP_PCT_PER_HOUR && cadence_h <= 1) compliance_flags.push('ABOVE_HL_FUNDING_CAP');
  if (Math.abs(simple_annual_pct) > 500) compliance_flags.push('EXTREME_ANNUALIZED_RATE');

  // Cross-venue arb: long venue1 (receive funding) + short venue2 (pay funding) -- delta neutral
  let cross_venue_arb = null;
  const venue2 = (pp.venue2 || '').toLowerCase();
  if (venue2 && venue2 !== venue) {
    const v2_def    = VENUE_FEES[venue2] || VENUE_FEES.binance;
    const v2_cadh   = Math.max(0.0001, safeNum(pp.venue2_cadence_hours, v2_def.cadence_hours));
    const v2_rate   = safeNum(pp.venue2_funding_rate_pct_per_period, 0.03);
    const v2_taker  = Math.max(0, safeNum(pp.venue2_taker_fee_pct, v2_def.taker_pct));
    const v2_hourly = round6(v2_rate / v2_cadh);

    const diff_hourly        = round6(hourly_rate_pct - v2_hourly);
    const gross_simple_apr   = round4(diff_hourly * HOURS_PER_YEAR);
    const gross_compound_apr = round4(compoundAnnualPct(diff_hourly));
    // 4 trades total: open+close on each venue
    const round_trip_fee     = round6(2 * (taker_pct + v2_taker));
    const daily_diff         = diff_hourly * 24;
    const days_to_break_even = daily_diff > 0
      ? round2(round_trip_fee / (daily_diff / 100))
      : null;
    const arb_viable = diff_hourly > 0 && gross_simple_apr > round_trip_fee;
    if (!arb_viable) compliance_flags.push('ARB_NOT_VIABLE_AFTER_FEES');

    cross_venue_arb = {
      venue1: venue,
      venue1_hourly_rate_pct: round6(hourly_rate_pct),
      venue2: venue2,
      venue2_hourly_rate_pct: round6(v2_hourly),
      differential_hourly_pct: diff_hourly,
      gross_simple_arb_apr_pct: gross_simple_apr,
      gross_compound_arb_apr_pct: gross_compound_apr,
      round_trip_fee_pct: round_trip_fee,
      days_to_break_even,
      arb_viable,
      note: 'Long venue1 (receive funding) + short venue2 (pay funding). Delta-neutral when position sizes match. Profitable when venue1_hourly > venue2_hourly and differential APR > round-trip fees.',
    };
  }

  // Delta-neutral carry: hold DeFi yield asset + short perp to neutralise price risk
  let delta_neutral_carry = null;
  const defi_yield = safeNum(pp.defi_yield_pct_annual, NaN);
  if (Number.isFinite(defi_yield) && defi_yield >= 0) {
    const total_simple   = round4(simple_annual_pct + defi_yield);
    const total_compound = round4(compound_annual_pct + defi_yield);
    const hedge_fee      = round6(2 * taker_pct);
    delta_neutral_carry = {
      perp_funding_simple_apr_pct:   round4(simple_annual_pct),
      perp_funding_compound_apr_pct: round4(compound_annual_pct),
      defi_yield_apy_pct:            round4(defi_yield),
      total_carry_simple_pct:        total_simple,
      total_carry_compound_pct:      total_compound,
      hedge_open_close_fee_pct:      hedge_fee,
      strategy_note: 'Hold collateral asset (e.g. Ethena sUSDe) for DeFi yield. Short perp hedges spot price exposure. Net APR = funding received + collateral yield. Fee drag = 2x taker fee for open+close of the hedge.',
    };
  }

  const output_payload = {
    venue,
    asset: String(pp.asset || 'BTC'),
    funding_rate_pct_per_period: round6(rate_pct),
    cadence_hours: round6(cadence_h),
    hourly_rate_pct,
    simple_annualized_rate_pct: simple_annual_pct,
    compound_annualized_rate_pct: compound_annual_pct,
    basis_spot_price: round6(basis_spot),
    basis_perp_price: round6(basis_perp),
    basis_usd,
    basis_pct,
    annualized_basis_pct,
    cross_venue_arb,
    delta_neutral_carry,
    hl_funding_cap_pct_per_hour: HL_FUNDING_CAP_PCT_PER_HOUR,
