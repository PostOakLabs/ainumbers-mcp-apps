...
**Idempotency & retry (RUN-1-1).** Every tool call is deterministic: a safe retry is a re-run that returns a byte-identical artifact (`generated_at`/`opened_at` differ; hashes do not), and `run_chain` echoes a stable `dedupe: { input_hash, composite_execution_hash }` for client-side dedupe. Dedupe is a cache concern, not a correctness concern. The not-stateless carve-outs (RenewalWatchWorkflow, `scheduled()`, `queue()`, MRTR_STATE_KEY requestState, the `input_required` flow, shared-cache hints, the null-strip live-vs-golden drift class) are documented in [`docs/IDEMPOTENCY.md`](docs/IDEMPOTENCY.md); the KV/Cache receipt cache (D2) stays deferred.
