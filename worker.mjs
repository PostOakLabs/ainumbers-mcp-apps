...
async function loadData(env) {
  ...
  // Workflow recipes (MCP-SUITE-RECIPES-1) — generated by generate.mjs. Tolerant load: a
  // missing file degrades suite_howto + recipe prompts to an honest "unavailable" instead of
  // taking the whole /mcp surface down with an asset-miss throw (the 2026-07-09 outage shape).
  // The generate.mjs self-check + precompute fail LOUD at build time, so this branch should
  ...
  // MCP-REACH-DISPATCH-1 D1: the non-node tool index (utility + PILOT widget tools, name +
  // description), generated by scripts/precompute-discovery.mjs. search-index.json is vendored from
  // the site repo and covers ChainGraph NODES only, so before this file find_tool could not name a
  // single utility tool — the ~48 tools a page-1-only host most needs to be told about were
...
// + suite_howto (MCP-SUITE-RECIPES-1 — the recipe how-to surface; deferring it would hide the
// first-call-correctness guidance this tool exists to provide) + call_tool (MCP-REACH-DISPATCH-1 D1
// — the read-only dispatcher that is the ONLY way a page-1-only host reaches the other 12 pages).
// Everything else is surfaced on demand through find_tool/find_chain/describe_tool + call_tool.
// ⛔ MCP-REACH-DISPATCH-1 D2: this set is ALSO the page-1 head — orderToolsHotFirst() below puts
// these 13 names at tools/list positions 1-13 on both the precompute and the runtime path, and
// scripts/check-hot-tool-order.mjs reds if any of them falls outside that window.
const HOT_TOOLS = new Set([
  ...
  'run_chain_batch',
  // MCP-REACH-DISPATCH-1 D1: the dispatcher itself. A host that reads only page 1 of tools/list
  // (Cursor, Codex, ChatGPT, Gemini CLI, Zed, Cline — measured 2026-09-24) can never load, and
  // therefore never call, a tool from pages 2-13; call_tool is the ONE listed tool that runs them.
  'call_tool',
]);

// MCP-REACH-DISPATCH-1 D1: the dispatcher's own name, in one place, so the allowlist generator,
// the self-target refusal and the dispatch-layer unwrap can never drift apart.
const CALL_TOOL_NAME = 'call_tool';

// MCP-REACH-DISPATCH-1 D1: the ONE steer sentence the discovery tools append, so a host that only
// loaded page 1 learns the route to a name it cannot see in its own tool list. Byte-identical text
// on find_tool, find_chain and describe_tool by construction (one constant, three call sites).
const DISPATCH_STEER = 'If a tool named above is not in your tool list, call ' + CALL_TOOL_NAME + ' with { name, arguments } to run it anyway.';

// MCP-REACH-DISPATCH-1 D2 — `initialize.instructions` (R-P9: there were none). Passed to McpServer
// so the SDK serves it natively, which means the precomputed static initialize.json captures it at
...
const dispatchSteerFor = (name) => 'If ' + name + ' is not in your tool list, call ' + CALL_TOOL_NAME + ' with { name, arguments } to run it.';

// MCP-REACH-DISPATCH-1 D2 — THE ONE ordering function, used by BOTH the build-time precompute
// (scripts/precompute-discovery.mjs) and the runtime SDK fallback path (tools/list branch below).
// Registration order put the hot set at page-1 positions 17-34 by accident (R-P6); a host that
// truncates the advertised list before position 34 lost find_tool/call_tool and with them every
// route to the rest of the estate. Stable: hot names in HOT_TOOLS iteration order first, then
// everything else in its original (registration) order — so the only observable move is the head.
function orderToolsHotFirst(tools) {
  const hot = [];
  const rest = [];
  const byName = new Map(tools.map((t) => [t.name, t]));
  for (const name of HOT_TOOLS) {
    const t = byName.get(name);
    if (t) hot.push(t);
  }
  const hotSeen = new Set(hot.map((t) => t.name));
  for (const t of tools) if (!hotSeen.has(t.name)) rest.push(t);
  return [...hot, ...rest];
}
...
const SUITE_CONVENTIONS =
  'Suite conventions: every AINumbers tool is read-only and deterministic, so the same inputs always yield the same execution_hash. ' +
  'Export the Policy Mandate (JSON + Markdown) at each stage and pass prior stage outputs forward as the next stage\'s inputs. ' +
  'Verify any artifact with verify_execution_hash (it recomputes the RFC 8785/JCS SHA-256 over {policy_parameters, output_payload} — never trust an unverified hash), and aggregate a multi-tool session with build_session_receipt for one tamper-evident audit object. ' +
  'Zero PII: use synthetic data only.';
...
// missing either annotation is EXCLUDED — the file fails closed by construction.
// ⛔ FAIL CLOSED AT READ TIME TOO: an asset miss or a malformed file yields an EMPTY set, and an
// empty set refuses every dispatch. The reason is the approval model: a proxied call is approved
// once (as call_tool) and so skips the host's per-tool consent prompt, which is only safe for tools
// that change nothing and reach nothing outside. "Allowlist unavailable" must therefore never
// degrade to "allow".
let _dispatchAllowlist = null;
async function getDispatchAllowlist(env) {
  if (_dispatchAllowlist) return _dispatchAllowlist;
  try {
    const r = await env.ASSETS.fetch('https://assets.local/mcp/dispatch-allowlist.json');
    if (!r.ok) return (_dispatchAllowlist = new Set());
    const j = await r.json();
    const names = Array.isArray(j?.tools) ? j.tools.filter((n) => typeof n === 'string' && n !== CALL_TOOL_NAME) : [];
...
// Re-applies mcp-apps#294 (MCP-TOOLSLIST-PAGINATION-1, 2026-08-29) onto the current master by
// hand — the PR CONFLICTed on worker.mjs after 636+ lines of master drift, so the LOGICAL change
// was re-applied, never merged. The live tools/list reply is 2.24MB (719 tools) in ONE frame —
// thin clients and scanners that cap or time out on large bodies fail to consume those bytes at
// all (mcpqueen scored 62/C, down from 100/A, reporting "tools/list failed: HTTP 200" on exactly
// this). The MCP spec's answer is cursor pagination: a page result carries `nextCursor`, the
// client echoes it as `params.cursor` to fetch the next page, and the page that ends the list
// omits `nextCursor`.
//
// ARCHITECTURE — do not regress the reason the static path exists: the templates are PRE-FRAMED
// TEXT precisely so a cold isolate never JSON.parses / re-stringifies the 2.24MB body (that parse
...
// Page budget in TEMPLATE bytes. Measured on the 2026-09-17 template: 719 tools, mean 3,118B /
// p50 3,081B / max 9,278B per tool → 16 pages of 26,368-149,132B carrying 8-73 tools each —
...
// ── RUN-4-1 (2026-09-22): keyset hardening of the page token + `total` echo on continuations ───
// The shipped v1 token is a raw BYTE OFFSET into the template. The templates are regenerated by
// the vendor pipeline (precompute-discovery.mjs captures SDK REGISTRATION order — measured on the
// 2026-09-21 vendored templates: not sorted by name), so a regen that inserts/removes/reorders
// elements shifts downstream byte offsets. A stale v1 token usually fails the boundary check below
// and is cleanly refused — but it can also land EXACTLY on a DIFFERENT element boundary of the
// regenerated template: accepted, then silently serving the wrong page (skip/dup). v2 closes that
// class: the token carries the ID of the page's first element, and resolution looks that id up in
// a memoized per-template INDEX (below), so the page anchor is IDENTITY, not position — stable
...
// the static path exists to avoid) — built lazily on the first cursor-carrying request per
// template per isolate and memoized: the default (no-cursor) path never pays it, a paginating
// client pays it once, not per page. The same walk yields the element count, echoed as `total` on
// continuation pages ONLY (fence: no default-shape changes — asserted by
// scripts/test-mcp-pagination-keyset.mjs).
const LIST_KEYSET_PREFIX = 'v2.';
// First key of each template's element objects — the regen-stable identity a v2 token carries.
// Measured on the vendored templates: tools/prompts elements open {"name":…, resources elements
// open {"uri":…; ids are unique per template (check-tool-names gate for tools).
const LIST_ID_FIELD = {
  'tools': 'name',
  'resources': 'uri',
  'prompts': 'name',
...
// Value of the FIRST member of the element opening at `at` (must point at '{') when its key is
// exactly `want`. Elements are machine-generated with the id field first; any other shape —
// escaped bytes, a different first key — returns null and DEGRADES the whole template to the
// shipped v1 offset tokens (never a wrong answer, just the pre-RUN-4-1 behaviour).
function firstIdFieldValue(tpl, at, want) {
  let i = at + 1;
  if (tpl[i] !== '"') return null;
  let j = i + 1;
  while (j < tpl.length && tpl[j] !== '"') { if (tpl[j] === '\\') return null; j++; }
  if (tpl[j] !== '"' || tpl[j + 1] !== ':' || tpl[j + 2] !== '"') return null;
  if (tpl.slice(i + 1, j) !== want) return null;
  let k = j + 3;
  let out = '';
  while (k < tpl.length) {
    const c = tpl[k];
    if (c === '\\') return null;          // ids are generated slugs/uris — never escaped
    if (c === '"') return out;
    out += c;
    k++;
  }
...
// Memoized per-template element index: { total, starts, ids, byStart } — built by ONE full
// string/escape/depth-aware walk of the list array (same discipline as scanListPageBounds;
// O(template) bytes, NO JSON.parse). See the RUN-4-1 block above for the lazy/memoized budget.
// A null value is cached too: a template that does not match the expected generated shape must
// not re-pay the walk on every cursor request.
const _listIndex = new Map();
function getListIndex(tpl, key, indexKey) {
  if (_listIndex.has(indexKey)) return _listIndex.get(indexKey);
  let idx = null;
  const arrayStart = locateListArrayStart(tpl, key);
  const field = LIST_ID_FIELD[key];
  if (arrayStart >= 0 && field) {
    const starts = [];
    const ids = [];
    let depth = 0, inStr = false, esc = false, elemStart = -1, broken = false;
    for (let i = arrayStart; i < tpl.length; i++) {
      ...
      if (c === '}' || c === ']') {
        if (c === ']' && depth === 0) break;   // end of the list array
        depth--;
        if (depth === 0 && c === '}') {
          const id = firstIdFieldValue(tpl, elemStart, field);
          if (id === null) { broken = true; break; }
          starts.push(elemStart);
          ids.push(id);
        }
      }
    }
    if (!broken && starts.length) {
      const byStart = new Map();
      for (let i = 0; i < starts.length; i++) byStart.set(starts[i], i);
      idx = { total: starts.length, starts, ids, byStart };
    }
  }
  _listIndex.set(indexKey, idx);
  return idx;
}
...
function buildListPage(tpl, key, cursor, indexKey) {
  const arrayStart = locateListArrayStart(tpl, key);
  if (arrayStart < 0) return null;
  // The index exists only on the cursor-carrying path (RUN-4-1 budget: page one never pays the
  // walk — and stays byte-identical, v1 token included).
  const idx = cursor === undefined ? null : getListIndex(tpl, key, indexKey ?? key);
  let from = arrayStart;
  if (cursor !== undefined) {
    if (typeof cursor !== 'string') return LIST_CURSOR_INVALID;
    if (cursor.startsWith(LIST_KEYSET_PREFIX)) {
      // v2: anchor by identity in the current template (regen-stable — RUN-4-1 block above).
      const id = cursor.slice(LIST_KEYSET_PREFIX.length);
      if (!idx || !/^[A-Za-z0-9_.:\/-]{1,200}$/.test(id)) return LIST_CURSOR_INVALID;
      const at = idx.ids.indexOf(id);
  ...
  // `total` echo (RUN-4-1): cursor-carrying pages only — INCLUDING the final page (a walk that
  // ends the array is still a cursor-carrying reply). Guarded splice into the generated envelope
  // head — if the expected `"result":{"resultType":"complete",` prologue is not found, the total
  // is silently omitted (never a malformed frame). No-cursor page one keeps the shipped bytes.
  let headOut = head;
  if (idx) {
    const prologue = '"result":{"resultType":"complete",';
    const at = head.lastIndexOf(prologue);
    if (at >= 0) headOut = head.slice(0, at) + '"result":{"total":' + idx.total + ',' + prologue.slice('"result":{'.length) + head.slice(at + prologue.length);
  }
  if (b.next >= 0) {
    ...
    if (!tpl.endsWith(']}}\n\n')) return null;
    // Continuation token: v2 keyset (id of the first element of the NEXT page — scanListPageBounds
    // hands us exactly that element start in b.next) whenever the index is available; degrade to
    // the shipped v1 offset token when it is not. Page one (no cursor → no index) keeps issuing
    // v1 so the default reply's bytes are unchanged.
    let nextToken = LIST_CURSOR_PREFIX + b.next;
    if (idx) {
      const at = idx.byStart.get(b.next);
      if (at !== undefined) nextToken = LIST_KEYSET_PREFIX + idx.ids[at];
    }
    return headOut + slice + '],"nextCursor":' + JSON.stringify(nextToken) + '}}\n\n';
  }
  if (b.arrayEnd < 0) return null;
  return headOut + slice + tpl.slice(b.arrayEnd); // no-cursor: byte-identical full-frame recomposition
}
...
function describeToolPayload(data, def) {
  return {
    ...
    // MCP-REACH-DISPATCH-1 D1: describe_tool is the tool a host calls about a name it CANNOT see in
    // its own list (that is the whole point of the trimmed list + find_tool flow), so the route to
    // running it belongs in the same answer. Declared in describe_tool's outputSchema.
    dispatch_hint: dispatchSteerFor(def.name),
  };
}
...
// tools/list pagination (spec 2025-06-18 §Pagination — OPTIONAL). The page size deliberately
// stays ≥ the current surface so every existing client keeps receiving the WHOLE list in one
// response with no nextCursor. The cursor is an opaque decimal offset into the build-stable
// static tools ordering. MCP-TOOLSLIST-PAGINATION-2 (2026-09-17): the STATIC fast path now
...
// MCP-REACH-DISPATCH-1 D1 — non-node tool matcher for find_tool. Input is the generated
// data/mcp/utility-tool-index.json ({ tools: [{ name, title, description }] }); scoring is plain
// query-term overlap over name+title+description, name matches weighted double, ties broken by
// index order. Deliberately NOT a second BM25 index (see the call site): O(48 × terms) string work,
// no per-request statistics to rebuild. A missing/malformed file yields [] — find_tool degrades to
// its node-only answer exactly as before this row.
function matchUtilityTools(query, utilityIndex, topN = 3) {
  const entries = Array.isArray(utilityIndex?.tools) ? utilityIndex.tools : [];
  if (!entries.length) return [];
  const terms = String(query ?? '').toLowerCase().replace(/[^a-z0-9_-]/g, ' ').split(/\s+/).filter((t) => t.length > 1);
  ...
  for (const e of entries) {
    ...
    const textHay = (name + ' ' + String(e?.title ?? '') + ' ' + String(e?.description ?? '')).toLowerCase();
    let score = 0;
    for (const t of terms) {
      if (nameHay.includes(t)) score += 2;
      else if (textHay.includes(t)) score += 1;
    }
...
function buildServer({ manifests, widgets, loadWidget, catalog, chaingraph, searchIndex, chainFixtures, fvStatusIndex, recipes, showcasePrompts, describeMap, lifecycle, utilityIndex }, { onlyTool = null, mrtr = null, requestId = null } = {}) {
  ...
  const fvStatusNote = fvStatusEntries.length === 1
    ...
    : '';
  const server = new McpServer({ name: 'ainumbers-apps', version: '1.2.0' }, { instructions: SERVER_INSTRUCTIONS });
  ...
  server.registerTool('list_ainumbers_tools', {
    ...
    inputSchema: {
      ...
      cursor: z.string().optional().describe('Page token: echo the nextCursor from the previous reply to fetch the next page (keyset on tool name — stable across catalog regens).'),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, async ({ query, category, limit, cursor }) => {
    const q = (query ?? '').toLowerCase();
    // RUN-4-1: the full match set is computed BEFORE pagination so `total` reflects the whole
    // result the filters select (the shipped code sliced first and had no total at all).
    ...
    // Keyset pagination on tool name (the catalog's regen-stable identity: unique per the
    // check-tool-names gate). The token names the LAST row served; the next page serves strictly
    // after it, so catalog regens (insertions/removals/reorders) cannot shift membership the way
    // an offset cursor would. An unknown token (row removed by a regen) is an explicit error
    // result — never a silently shifted page.
    let startIdx = 0;
    ...
    const rows = filtered
      .slice(startIdx, startIdx + (limit ?? 20))
    ...
    const out = { count: rows.length, total, tools: rows };
    const last = rows[rows.length - 1];
    if (last && startIdx + rows.length < total) out.nextCursor = last.name;
    return { content: [{ type: 'text', text: JSON.stringify(rows, null, 2) }], structuredContent: out };
  });
  ...
  // mode:"estimate" shipped FIRST: validates every row (chain exists, step counts,
  ...
  // mode:"run" executes each row through executeChainRun — the SAME engine as
  // run_chain — and reports a PER-ROW terminal status ('completed' | 'input_required'
  // | 'escalated' | 'partial' | 'error' | 'unknown_chain' | 'no_steps'): one row's
  // failure never fails another, and all rows' results ride the ONE response.
  //
  // Caps are OURS, measured LIVE on the deployed free-plan endpoint
  // (https://mcp.ainumbers.co/mcp, 2026-09-22T03:0xZ, probe kept tiny per the row's
  // back-off rule): an N-step fixture-backed chain via run_chain shares ONE
  // invocation CPU budget across N kernel builds — the same bound an N-row batch
  // hits. Chains of 1, 2, 3, 4, 5, 6 and 11 steps ALL completed green in one
  // invocation (worst case 416 ms / 138 KB response; every probe HTTP 200 with a
  // full composite_execution_hash; no error -> no back-off needed, and the probe
  // deliberately stopped there rather than hunting the wall). Measured caps:
  //   RUN_CHAIN_BATCH_MAX_ROWS = 8            — single-digit rows; each row returns
  //                                             its full composite artifact.
  //   RUN_CHAIN_BATCH_MAX_TOTAL_STEPS = 11    — the largest server-kernel-step count
  //                                             MEASURED completing in one invocation.
  //                                             A bound from evidence, NOT a claim
  //                                             about where the wall actually is.
  // These are measured caps, not copied concurrency defaults. JSON-RPC-level
  // batching stays REFUSED (-32600): this is ONE tools/call, not a batched
  // JSON-RPC array.
  //
  // Retry semantics: a client retry re-runs the WHOLE batch. Safe because every
  // chain is deterministic — same inputs -> same per-step outputs, same
  // composite_execution_hash (wall-clock envelope fields like generated_at differ;
  // nothing hashed does). See docs/client-fan-out-patterns.md for the blessed
  // sequential-calls pattern beyond the cap. Mandates and input_required
  // escalation are NOT accepted on batch rows (keeps the sync path CPU-bounded);
  // an escalated row always carries the non-blocking resolve_handle record.
  // -------------------------------------------------------------------------
  const RUN_CHAIN_BATCH_MAX_TOTAL_STEPS = 11;
  server.registerTool('run_chain_batch', {
    title: 'Run or estimate several ChainGraph chains in one call',
    description:
      'Batch over named chains in ONE round-trip with a PER-ROW terminal status — one row\'s failure never fails ' +
      'another, and every row\'s result rides the single response. mode:"run" (default) executes each row through the ' +
      'same engine as run_chain (composite artifact + execution_hash per row, deterministic); mode:"estimate" validates ' +
      'every row (chain exists, step counts, compute feasibility, inputs coverage, OCG §21.4 gate rule previews) WITHOUT ' +
      'executing anything. Capped at ' + RUN_CHAIN_BATCH_MAX_ROWS + ' rows and ' + RUN_CHAIN_BATCH_MAX_TOTAL_STEPS + ' total server-kernel steps per batch — ' +
      'caps MEASURED live on the deployed free-plan endpoint, where N kernel builds in one invocation share one CPU ' +
      'budget (an 11-step chain: green at 416 ms). Beyond the cap, use sequential run_chain calls — each call gets its ' +
      'own CPU budget (the blessed fan-out pattern on this tier; docs/client-fan-out-patterns.md). Retry = re-run the ' +
      'WHOLE batch: safe, because determinism reproduces byte-identical execution hashes. Mandates and input_required ' +
      'escalation are not accepted on batch rows; escalated rows carry the non-blocking resolve_handle record. ' +
      'Zero PII, zero payload logging.',
    inputSchema: {
      mode: z.enum(['estimate', 'run']).optional()
        .describe('"run" (default) executes every row server-side through the same engine as run_chain; "estimate" validates all rows and previews gates without executing anything.'),
      rows: z.array(z.object({
        ...
        inputs: z.record(z.record(z.any())).optional()
          .describe('Map of step tool_id -> policy_parameters for THIS row (same shape as run_chain inputs). Omitted steps run with fixture defaults; kernels needing required fields come back status "input_required", never failed silently.'),
      })).min(1).max(RUN_CHAIN_BATCH_MAX_ROWS)
        .describe('1-' + RUN_CHAIN_BATCH_MAX_ROWS + ' rows, one chain per row. The cap keeps the synchronous response bounded and inside the free-plan CPU budget measured live; for larger fan-out use sequential run_chain calls (docs/client-fan-out-patterns.md).'),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, async ({ mode, rows }) => {
    const effectiveMode = mode ?? 'run';

    // Shared CPU-light validation (pure lookups over already-loaded data — no kernel runs).
    const validated = rows.map((row, i) => {
      const idx = i + 1;
      const chainMeta = namedChains[row.chain];
      if (!chainMeta) return { index: idx, chain: row.chain, status: 'unknown_chain', hint: 'List chains with find_chain or build_workflow_links.' };
      const chainSteps = chainMeta.steps ?? [];
      if (!chainSteps.length) return { index: idx, chain: row.chain, status: 'no_steps' };
      return { index: idx, chain: row.chain, chainMeta, chainSteps, row };
    });
    const serverStepsOf = (chain, chainSteps) => (chainSteps ?? []).filter((s) => {
      const n = cgById[s.tool_id];
      return n && !n.gpu && !!getKernel(s.tool_id);
    }).length;
    const totalServerSteps = validated.reduce((acc, v) => acc + (v.chainMeta ? serverStepsOf(v.chain, v.chainSteps) : 0), 0);

    const capEcho = {
      max_rows: RUN_CHAIN_BATCH_MAX_ROWS,
      max_total_server_kernel_steps: RUN_CHAIN_BATCH_MAX_TOTAL_STEPS,
      measured_live: '2026-09-22 on https://mcp.ainumbers.co/mcp (deployed free-plan endpoint): fixture-backed chains of 1, 2, 3, 4, 5, 6 and 11 steps each completed via run_chain in one invocation (worst case 416 ms / 138 KB; all HTTP 200 with full composite_execution_hash). 11 kernel builds per invocation is the largest MEASURED green count — the probe stopped there on purpose — so the total step budget is set to 11. Row cap 8 keeps each row\'s full composite artifact inside a bounded synchronous response. Measured caps, not copied concurrency defaults.',
    };

    // ── mode:"estimate" — the absorbed COSTPREFLIGHT-1. No kernel executes. ──────
    if (effectiveMode === 'estimate') {
      const results = validated.map((v) => {
        if (v.status) return v; // unknown_chain / no_steps already carry their terminal status
        const steps = v.chainSteps.map((s, si) => {
          const tid = s.tool_id;
          const node = cgById[tid];
          ...
          const callerPp = v.row.inputs?.[tid];
          const fixturePp = chainFixtures?.[v.chain]?.[tid];
          const stepEst = {
            order: si + 1,
            tool_id: tid,
        ...
        return {
          ...
          chain: v.chain,
          status: 'ready',
          step_count: v.chainSteps.length,
      ...
      const out = {
        ...
        within_step_budget: totalServerSteps <= RUN_CHAIN_BATCH_MAX_TOTAL_STEPS,
        cap: capEcho,
        summary: {
    ...
    // ── mode:"run" — small-N sync batch, per-row terminal status ─────────────────
    // Budget refusal BEFORE any execution (CPU-light): the whole batch must fit the
    // live-measured per-invocation kernel-step budget.
    if (totalServerSteps > RUN_CHAIN_BATCH_MAX_TOTAL_STEPS) {
      const errOut = {
        error: 'batch_step_budget_exceeded',
        detail: 'This batch needs ' + totalServerSteps + ' server-kernel steps in one invocation; the measured budget is ' + RUN_CHAIN_BATCH_MAX_TOTAL_STEPS + '. N kernel builds share ONE free-plan CPU budget per invocation — measured live on the deployed endpoint (cap comment on run_chain_batch). Split the batch, drop rows, or use sequential run_chain calls (docs/client-fan-out-patterns.md).',
        total_server_kernel_steps: totalServerSteps,
        max_total_server_kernel_steps: RUN_CHAIN_BATCH_MAX_TOTAL_STEPS,
      };
      return { isError: true, content: [{ type: 'text', text: JSON.stringify(errOut, null, 2) }], structuredContent: errOut };
    ...
    const results = [];
    const summary = { completed: 0, input_required: 0, escalated: 0, partial: 0, error: 0, unknown_chain: 0, no_steps: 0 };
    for (const v of validated) {
      if (v.status) { // unknown_chain / no_steps: terminal for THIS row only
        const row = { index: v.index, chain: v.chain, terminal_status: v.status };
        if (v.hint) row.hint = v.hint;
        results.push(row);
        summary[v.status]++;
        continue;
      }
      try {
        // Same engine, same defaults as a direct run_chain call — resolve_handle escalation
        // (non-blocking), auto compute, no mandate. Nothing here can block the batch.
        const rowRes = await executeChainRun({
          chain: v.chain,
          inputs: v.row.inputs,
          compute: 'auto',
          mandate: undefined,
          escalation_transport: 'resolve_handle',
        });
        const out = rowRes.structuredContent ?? {};
        const stepStatuses = (out.steps ?? []).map((s) => s.status);
        let terminal;
        if (out.status === 'escalated') terminal = 'escalated';
        else if (stepStatuses.includes('input_required')) terminal = 'input_required';
        else if (stepStatuses.length && stepStatuses.every((s) => s === 'ok' || s === 'skipped_by_gate')) terminal = 'completed';
        else terminal = 'partial';
        // Trim the per-row OTel span document (a base64 data: URI, tens of KB) from the batch
        // response — response metadata, regenerable by calling run_chain directly for that row.
        // Every hashed byte stays.
        if (out.otel_span_link) delete out.otel_span_link;
        const row = {
          index: v.index,
          chain: v.chain,
          terminal_status: terminal,
          step_count: out.step_count ?? null,
          steps_ran: out.steps_ran ?? null,
          composite_execution_hash: out.composite_execution_hash ?? null,
          result: out,
        };
        if (out.ledger_url) row.ledger_url = out.ledger_url;
        results.push(row);
        summary[terminal]++;
      } catch (err) {
        results.push({ index: v.index, chain: v.chain, terminal_status: 'error', error: String(err?.message ?? err) });
        summary.error++;
      }
    }
    const out = {
      ...
      summary,
      results,
      retry_semantics: 'A retry re-runs the WHOLE batch. Safe: every chain is deterministic — same inputs reproduce the same per-step outputs and the same composite_execution_hash (only wall-clock envelope fields like generated_at differ). Fill inputs for any "input_required" row before retrying; escalated rows need no retry (the run completed; see result.escalation_resolve).',
      beyond_cap: 'For fan-out beyond the cap, make sequential run_chain calls — each call gets its own free-plan CPU budget (the blessed pattern on this tier; docs/client-fan-out-patterns.md). Async/Workflows-based batch is on WATCH (paid tier / design row).',
      note: 'Batch semantics inspired by GREP AI (grep.ai); caps are ours, measured live.',
    };
    return { content: [{ type: 'text', text: JSON.stringify(out, null, 2) }], structuredContent: out };
  });
  ...
  server.registerTool('find_tool', {
    ...
    // MCP-REACH-DISPATCH-1 D1: the non-node leg. searchIndex.nodes is the site-vendored NODE index,
    // so utility/widget tools (call_tool, run_chain, build_evidence_pack, the PILOT widgets, ...)
    // never appeared in any find_tool answer. Term-overlap ranking over the generated
    // data/mcp/utility-tool-index.json rather than a second BM25 index: the vendored index carries
    // precomputed idf/tf statistics, and recomputing those per request on the Free plan's ~10ms CPU
    // budget is exactly the cost the static-discovery architecture exists to avoid.
    const utilityMatches = matchUtilityTools(query, utilityIndex, 3);
    ...
    return {
      content: [{ type: 'text', text: JSON.stringify(utilityMatches.length ? { tools: out, utility_tools: utilityMatches } : out, null, 2) + '\n' + DISPATCH_STEER }],
      structuredContent: payload,
    };
  });

  // -------------------------------------------------------------------------
  // call_tool (MCP-REACH-DISPATCH-1 D1) — the read-only dispatcher. Measured 2026-09-24: the live
  // tools/list is 13 pages / 722 tools and six hosts (Cursor, Codex, ChatGPT, Gemini CLI, Zed,
  // Cline) issue exactly ONE tools/list request, so 647 tools were unreachable for them — a host
  // refuses to call a tool it never loaded, so "the server would run it" was never enough.
  //
  // ⛔ THE EXECUTION PATH IS NOT RE-IMPLEMENTED HERE. On the HTTP worker the dispatch layer
  // (see the MCP-REACH-DISPATCH-1 D1 block in fetch() below) rewrites the JSON-RPC params to the
  // TARGET tool and lets the request continue down the SAME single-tool build + SDK validation +
  // kernel path a direct tools/call takes. That is what makes execution_hash byte-identical and
  // makes a schema-invalid argument fail exactly as it fails on a direct call: there is only ever
  // one validator and one executor. This SDK callback is the direct-transport (in-process /
  // stdio / precompute) leg, where there is no dispatch layer to rewrite anything — it refuses
  // rather than growing a second execution path (same posture as describe_tool's map-miss throw).
  // -------------------------------------------------------------------------
  server.registerTool(CALL_TOOL_NAME, {
    title: 'Run any AINumbers tool by name',
    description:
      'Runs ONE read-only AINumbers tool that is not in your tool list. tools/list is paginated (13 pages, 722 tools) and many hosts read only the first page; this is the door to the rest. ' +
      'Pass { name: "<exact mcp_name>", arguments: { ... } } — the target\'s own inputSchema is validated and its result is returned verbatim, including execution_hash, so a dispatched call and a direct call are byte-identical. ' +
      'Get a name from find_tool (single calculators), find_chain (workflows) or describe_tool (exact schema). ' +
      'Read-only tools only: anything that issues a credential, stamps an anchor or reaches the network is refused here and must be called directly so your host can approve it.',
    inputSchema: {
      name: z.string().describe('Exact mcp_name of the tool to run (e.g. "recompute_payment_waterfall"). Use find_tool/describe_tool to get it; this takes the exact name only.'),
      arguments: z.record(z.string(), z.unknown()).optional().describe("The target tool's own arguments object, exactly as you would pass it on a direct call. Omit for a no-argument tool."),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, ({ name }) => {
    throw new McpError(ErrorCode.InvalidParams,
      CALL_TOOL_NAME + ': dispatch is served by the HTTP worker request layer, which rewrites the call to "' + String(name) + '" and runs the one real execution path. This in-process transport has no dispatch layer — call "' + String(name) + '" directly here.');
  });
  ...
  // recipes in one response. Data: data/mcp/recipes.json (generate.mjs
  // precompute of the mcp.html #workflows table — the same SSOT counts.mjs
  // derives the published workflows.recipes count from — enriched with
  // chaingraph.json chains). The per-recipe MCP prompts are the spec-native
  // home for the same guidance; this tool is the fallback for clients that
  // don't surface prompts.
  // -------------------------------------------------------------------------
  server.registerTool('suite_howto', {
    title: 'AINumbers suite how-to: workflow recipes',
    description:
      'How to chain AINumbers tools into audited workflows: call with NO recipe_id for the compact recipe index, or with one recipe_id for that workflow\'s full step-by-step recipe. ' +
      'Every recipe carries the suite conventions end to end: Policy Mandate exports, execution_hash verification, and receipts.',
    inputSchema: {
      recipe_id: z.string().optional().describe('A recipe id from the no-arg index (e.g. "aml-programme"). Omit for the compact index; pass exactly ONE id per call.'),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, ({ recipe_id }) => {
    ...
    if (!recipe_id) {
      ...
      const index = {
        ...
        recipe_count: recipeList.length,
        recipes: recipeList.map((r) => ({ id: r.id, title: r.title ?? r.id, use_when: r.use_when })),
        suite_conventions: SUITE_CONVENTIONS,
        next_step: 'Call suite_howto with ONE recipe_id for the full step-by-step recipe. Each recipe is also registered as an MCP prompt (prompts/list) for clients that surface prompts.',
      };
      return {
    ...
    const full = {
      ...
      domain: r.domain,
      use_when: r.use_when,
      what_it_runs: r.what_it_runs,
      description: r.description,
      composer_url: r.composer_url,
      entry_mcp_name: r.entry_mcp_name,
      step_count: steps.length,
      steps,
      how_to_execute: [
        'Per-step: ' + (steps.some((s) => s.callable)
          ? 'for each step in order, if callable=true invoke its mcp_name via /mcp; if callable=false it is a browser tool, open tool_url.'
          : 'open each step\'s tool_url in order (browser tools; this recipe has no server-callable steps).'),
        'Orchestration: call build_workflow_links with chain "' + r.id + '" for the ordered prefill-ready deep-link set' + (r.composer_url ? ', or open the composer page: ' + r.composer_url : '') + '.',
      ],
      suite_conventions: SUITE_CONVENTIONS,
      source: 'mcp.html #workflows + chaingraph.json chains (generated; see data/mcp/recipes.json)',
    };
    return {
  ...
  //
  // MCP-SUITE-RECIPES-1: every suite workflow recipe is ALSO an MCP prompt — the spec-native
  // home for combine-guidance per the 2026-08-29 best-practices pass (Tim-approved row);
  // suite_howto is the tool-side fallback for clients that don't surface prompts. Same
  // generated source feeds both (data/mcp/recipes.json, generate.mjs precompute of the
  // mcp.html #workflows table — zero hand-written recipe text). Registered through regPrompt
  // AFTER the hand-authored flagships, so the richer flagship prompt wins any name collision
  // (first-registered-wins dedupe) and a duplicate can never 500 the /mcp handshake.
  // Descriptions are trigger-first ("Use when ...") per the row's design requirements.
  for (const r of (recipes?.recipes ?? [])) {
    const pName = r.id.replace(/-/g, '_') + '_workflow';
    regPrompt(pName, {
      title: (r.title ?? r.id) + ' (workflow recipe)',
      ...
      const stepLines = steps.length
        ? steps.map((s) =>
            '  ' + s.step + '. ' + s.display_name + ': ' +
            (s.callable ? 'call MCP tool `' + s.mcp_name + '`' : 'open ' + (s.tool_url ?? 'its tool page')) +
            (s.handoff ? '; then ' + s.handoff : '')
          ).join('\n')
        : '  Flow: ' + r.what_it_runs;
      const text =
        'Run the "' + (r.title ?? r.id) + '" workflow (recipe id "' + r.id + '").\n\n' +
        (r.description ? r.description + '\n\n' : '') +
        'Steps (' + steps.length + '):\n' + stepLines + '\n\n' +
        'Orchestration: call build_workflow_links with chain "' + r.id + '" for the ordered prefill-ready deep-link set' +
        (r.composer_url ? ', or open the composer page: ' + r.composer_url : '') + '.\n\n' +
        SUITE_CONVENTIONS;
      return {
        description: 'Guided walkthrough for the "' + (r.title ?? r.id) + '" workflow recipe (' + r.id + ').',
        messages: [{ role: 'user', content: { type: 'text', text } }],
      };
    });
  }
...
export { buildServer, widgetGlue, stripCspMeta, loadData, HOT_TOOLS, fragmentLink, lifecycleStatusOf, orderToolsHotFirst, CALL_TOOL_NAME };
...
// MCP-REACH-DISPATCH-1 D3, PR 1 of 2 (2026-09-25): MCP_PREPARSE_LIMITER (ns 3005, per-IP 60/60 s)
// is called HERE, pre-parse, IN ADDITION to 3001/3002 -- deliberately a no-op change in observable
// behaviour, because 3005 is looser than 3001 (60 vs 30 per IP per minute) and both are evaluated,
// so the binding cannot start refusing anything 3001 was not already refusing. Its only job in this
// PR is to PROVE the namespace is provisioned and answering on a live deploy; PR 2 then moves
// 3001/3002 behind the size-capped parse and leaves 3005 as the sole pre-parse check, with static
// discovery routed to MCP_DISCOVERY_GLOBAL_LIMITER (ns 3006) instead of the heavy 3001/3002 pair.
// ⛔ Do not collapse the two PRs: an un-provisioned ratelimit binding reads as undefined, and this
// function treats undefined as "never block" -- so a single-PR version could ship a discovery path
// with NO limiter at all and no signal that it had.
async function rateLimitExceeded(request, env) {
  const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
  const checks = [];
  if (env.MCP_PREPARSE_LIMITER) checks.push(env.MCP_PREPARSE_LIMITER.limit({ key: ip }));
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/mcp' || url.pathname === '/mcp/') {
      ...
      // `let`, not `const`: the MCP-REACH-DISPATCH-1 D1 unwrap below rewrites params.name to the
      // DISPATCH TARGET and re-derives this, so every downstream reader (known-set, onlyTool,
      // Analytics) sees the tool that actually ran — never the proxy.
      let toolName   = isToolCall ? (body?.params?.name ?? 'unknown') : null;
      let chainDepth = isToolCall ? (body?.params?.arguments?.chain_depth ?? 0) : null;
      // MCP-REACH-DISPATCH-1 D1: set to 'call_tool' once a dispatch has been unwrapped, and read
      // ONLY to stamp _meta["ainumbers/dispatched_via"] on the way out. It is deliberately outside
      // the hashed payload: execution_hash covers {policy_parameters, output_payload}, so a
      // dispatched call and a direct call hash identically, which is the row's parity gate.
      let dispatchedVia = null;
      ...
      if (body && body.id !== undefined && method === 'server/discover') {
        ...
        return new Response(
          JSON.stringify({ jsonrpc: '2.0', id: body.id, result: {
            ...
            // MCP-REACH-DISPATCH-1 D2: server/discover MIRRORS initialize.instructions — one
            // string, served from the same static capture, so a modern-era client (which learns
            // everything here and never calls initialize) and a legacy client are told the same
            // thing. ⛔ Do not re-diverge these two: the previous hand-written paragraph here was
            // longer, older, and silent about pagination and call_tool, which is exactly the
            // stranding R-P1/R-P9 measured. Task-shaped routing lives in find_chain/find_tool,
            // which the instructions point at.
      ...
      if (body && body.id !== undefined && STATIC_DISCOVERY_METHODS.has(method)) {
        try {
          if (method === 'initialize') {
            ...
            // MCP-REACH-DISPATCH-1 D2: `instructions` rides the static capture (the SDK emits it
            // from the McpServer option SERVER_INSTRUCTIONS, so precompute bakes the identical
            // bytes) — omitted, never empty-stringed, if an older static asset carries none.
          ...
          // (LIST_CURSOR_PREFIX) on page one and `v2.<element id>` (LIST_KEYSET_PREFIX, RUN-4-1
          // keyset hardening) on continuation pages. The SDK fallback's parseToolsCursorOffset()
      ...
      try {
        ...
        if (isToolCall && toolName) {
          ...
          // the identical remaining path a direct call takes: alias resolution already happened
          // above (and is applied to the target too), the Removed-lifecycle gate, the describe_tool
          // static branch, the single-tool build (`onlyTool`), the SDK's inputSchema validation and
          // the kernel itself all run exactly once, on the target. ⛔ Do NOT move this below the
          // isKnown/isRemoved reads — they must see the TARGET name, not 'call_tool'.
          // Refusals are TOOL RESULTS (isError:true, HTTP 200), one sentence each: a refusal is an
          ...
          if (toolName === CALL_TOOL_NAME) {
            ...
            body.params = { ...body.params, name: target, arguments: targetArgs ?? {} };
            toolName = target;
            chainDepth = body.params.arguments?.chain_depth ?? 0;
            dispatchedVia = CALL_TOOL_NAME;
          }
          ...
          if (isKnown && !isRemoved && toolName === 'describe_tool') {
            ...
            const result = { resultType: 'complete', content: [{ type: 'text', text: JSON.stringify(payload, null, 2) }], structuredContent: payload,
                             // MCP-REACH-DISPATCH-1 D1: describe_tool is answered on this static
                             // branch, so the dispatched_via stamp has to be applied here too — the
                             // SDK-path stamp below never sees this response.
                             ...(dispatchedVia ? { _meta: { 'ainumbers/dispatched_via': dispatchedVia } } : {}) };
            const sse = 'event: message\ndata: ' + JSON.stringify({ jsonrpc: '2.0', id: body.id, result }) + '\n\n';
            return frameResponse(sse, request, corsHeaders);
          }
        ...
        // tools/list post-processing on the SDK fallback path (lifecycle stamp + hot-first order).
        // ⛔ NO `defaultConfig:{defer_loading:true}` INJECTION ANY MORE (MCP-REACH-DISPATCH-1 D4).
        // The removed comment claimed "Anthropic Tool Search reads defaultConfig": it does not.
        // `defer_loading` is a CLIENT-side setting in both documented implementations (Anthropic
        // `mcp_toolset.default_config`, OpenAI `defer_loading` + `tool_search`); no host has ever
        // read it off a server's tools/list entry, and measured 2026-09-24 it cost 15,620 B of every
        // full walk for nothing. The lean-core intent it encoded is now served by page ORDER (D2:
        // the 13 hot tools occupy page-1 positions 1-13) plus call_tool, which actually works.
        let response;
        if (body?.method === 'tools/list') {
          try {
            ...
            if (Array.isArray(parsed?.result?.tools)) {
              ...
              // MCP-REACH-DISPATCH-1 D2 — the SAME ordering function the precompute uses, so this
              // fallback path and the static bytes agree on page 1 (the gate checks the static
              // bytes; a divergent fallback would be an unobserved second answer).
              parsed.result.tools = orderToolsHotFirst(parsed.result.tools);
            }
          ...
          try {
            ...
            // MCP-REACH-DISPATCH-1 D1: the ONLY difference between a dispatched result and a direct
            // one. `content`, `structuredContent`, `isError` and any existing `_meta` member (incl. a
            // future `_meta.proof` — CHECKPOINT-WORKER-1) pass through verbatim; this adds one key
            // to `_meta`, which is transport metadata and is not part of the hashed payload.
